...
MODULE moves_cbmc
  ...
  use energy_pairwise,only:energy,boltz,energyjc,calrhoz2,jccoru
  use energy_intramolecular,only:lininter_vib,lininter_bend,vtorso,U_bonded
  implicit none
  private
  save
  public::config,rosenbluth,schedule,explct,safeschedule,allocate_cbmc,init_cbmc,opt_safecbmc,output_cbmc_stats&
   ,output_safecbmc,read_checkpoint_cbmc,write_checkpoint_cbmc,align_lines,align_lines_partial_rigid_cbmc,dihedral_rigrot&
   ,group_cbmc_grow,align_planes_cbmc
...
!*****************************************************************
  subroutine config()
    use sim_particle,only:update_neighbor_list_molecule_v2,ctrmas,update_coord_in_tree
...
! end if
      !write(io_output,*) 'CK here just before rosenbluth!!!! Completed schedule!!!!!'  
! grow new chain conformation
      if (grouptype .eq. 1) then
         call group_cbmc_grow(.true.,lterm,i,imolty,ibox)
      else if (lrigid(imolty).and.icbdir(imolty).eq.1) then
         call rosenbluth(.true.,lterm,i,i,imolty,islen,ibox,igrow,vdum,lfixnow,cwtorfn,1,riutry(imolty,1),riutry(imolty,1)+1)
      else
...
! grow old chain conformation
      if (grouptype .eq. 1) then
         ...
         call rosenbluth(.false.,lterm,i,i,imolty,islen,ibox,igrow,vdum,lfixnow,cwtorfo,1,riutry(imolty,1),riutry(imolty,1)+1)
      else
...
! rxuion: 1= old configuration; 2= new configuration
      ...
      if(ljc(ibox)) then
          call energyjc(istt,iett,vTailo,imolty,ibox)
          vold(ivTail) = vTailo
          vold(ivInterLJ) = vold(ivInterLJ)+vTailo
          vold(ivTot) = vold(ivTot) + vTailo
          weiold = weiold * exp(-beta*vTailo)
          call calrhoz2(imolty,ibox,2)
          call calrhoz2(imolty,ibox,1)
          call energyjc(istt,iett,vTailn,imolty,ibox)
          vnew(ivTail) = vTailn
          vnew(ivInterLJ) = vnew(ivInterLJ)+vTailn
          vnew(ivTot) = vnew(ivTot) + vTailn
          weight = weight * exp(-beta*vTailn)
      end if
...
! write(99,*) 'cbmc softcut',i
        if(ljc(ibox)) call calrhoz2(imolty,ibox,3) 
        return
      end if
...
! we can now accept !!!!! ***
         ...
         vbox(ivTail,ibox) = vbox(ivTail,ibox) + ( vnew(ivTail) - vold(ivTail) )
...
! update linkcell, if applicable
         ...
         if (lneigh.or.lneighbor) then
            call update_neighbor_list_molecule_v2(i)
         end if
       else
          if(ljc(ibox)) call calrhoz2(imolty,ibox,3)
...
!>           0 = swatch moves for rigid molecules when nsampos=1
  subroutine rosenbluth(lnew,lterm,i,icharge,imolty,ifrom,ibox,igrow,wadd,lfixnow,cwtorf,movetype,first_bead,second_bead)
    ...
    logical::lflex2rig ! CK: flag of regrowing flexible chain before placing a rigid unit
    ...
    ! new stuff
    integer::itor,bin,counta,movetype,ku,orgmovetype
...
!   we don't want to do rigrot for rigid swatch when nsampos .ge. 3
    !write(io_output,*) 'CK here. movetype in Rosenb is', movetype !CK writes smt here...
    if (lrigid(imolty).and.movetype.ne.1) then
       wadd = 1.0E0_dp
       if (movetype.eq.2.or.movetype.eq.0.or.movetype.eq.5.or.movetype.eq.6) then
    ...
    if (lrigid(imolty).and.movetype.eq.1.and.icbdir(imolty).eq.1) then !CK:2EP/4EP
    !if (movetype.eq.1.and.icbdir(imolty).eq.1) then
       lflex2rig = .true.                                                                                   
    else
       lflex2rig = .false.
    end if
    !Swatch and swap the same from here change imovetype to 2 -- Jake Rafferty
    !CK added movetype 6 and exclude this type, need to check if you want to do safecbmc 
    if (movetype.gt.2.and.movetype.ne.6) movetype=2
! --- END JLR 11-24-09
       ...
       do iv = 1, invib(imolty,iu)
          ...
          if ( lexist(iu) .and. lexist(ju) ) then
             if ( lnew ) then
                ...
                if( isnan(xvec(iu,ju)).or.isnan(yvec(iu,ju)).or.isnan(zvec(iu,ju)) ) then ! CK added for nan issue
                write(io_output,*) 'CK here in determination of blength in new vectors.iu=',iu,'ju=',ju,'lnew=',lnew,&
                        'xvec=',xvec(iu,ju),'yvec=',yvec(iu,ju),'zvec=',zvec(iu,ju)
                write(io_output,*) 'imolty',imolty,'orgmovetype',orgmovetype
                write(io_output,*) 'ju part. rxnew=',rxnew(ju),'rynew=',rynew(ju),'yznew=',rznew(ju)
                write(io_output,*) 'iu part. rxnew=',rxnew(iu),'rynew=',rynew(iu),'yznew=',rznew(iu)
                call err_exit(__FILE__,__LINE__,'CK found nan issue in calculate all bond vectors for lexist',-1)
                end if ! CK added for nan issue end
             else
                ! use old coordinates
...
! *************************
    !if (lprint) then
    !    write(io_output,*) 'CK in cbmc b4 loop over trial units...ifrom',ifrom
    !end if
    do iw = 1, ifrom
       ...
       !write(io_output,*) 'CK is here b4 geom...iw',iw,'ifrom ',ifrom
       ...
       call geometry(lnew,iw,i,imolty,angstart,iuprev,glist,bondlen,bendang,phi,vvibtr,vbbtr,maxlen,wei_bend,lflex2rig)
       !end if 
       ...
       !if( lflex2rig.and.iw.eq.ifrom ) then ! CK grow the rigid unit by rigrot...2EP/4EP
       !         wadd = 1.0E0_dp
       !         first_bead = riutry(imolty,1)
       !         second_bead = riutry(imolty,1) + 1
       !         call rigrot(lnew,lterm,i,icharge,imolty,ibox,wadd,first_bead,second_bead,7)
       !else ! CK regular cbmc generation part
...
205       continue
          ...
          if (.not.lreturn.and.growprev(iw).eq.0) then
             ...
             if (lrigid(imolty).and.icbdir(imolty).eq.0) then ! CK: this is potentially a redundant part.
             ...
             !write(io_output,*) 'CK here. imolty',imolty,'length',length,'iuprev',iuprev,'iufrom',iufrom
             xub = xvec(iuprev,iufrom) / length
             yub = yvec(iuprev,iufrom) / length
             zub = zvec(iuprev,iufrom) / length
             call cone(1,xub,yub,zub,dum,dum)
             if ((movetype.eq.2.and.lring(imolty).and.iw.eq.1).or.movetype.eq.6) then !CK added movetype 6
                ltorsion = .false.
             else
                ltorsion = .true.
             end if
          ...
          if ( ltorsion ) then
             ...
             do itor=my_start,my_end
                ...
                if ( (.not. lnew) .and. ip .eq. 1 .and. itor .eq. 1) then ! original
                !if ( (.not. lflex2rig) .and. (.not. lnew) .and. ip .eq. 1 .and. itor .eq. 1) then ! CK modified
                ...
                !write(io_output,*) 'CK is inside ltorsion, angstart= ',angstart,' ntogrow= ',ntogrow
                do count = angstart,ntogrow 
                   if (.not. lflex2rig .or. count.eq.1 .or. iuprev.ne.riutry(imolty,1)) then ! CK added for 2EP/4EP
                   call cone(2,x,y,z,bendang(count),phi(count) + phidisp) 
                   if(isnan(x).or.isnan(y).or.isnan(z)) then ! CK debugging
                        write(io_output,*) 'CK here Torsional part NaN issue after cone...lnew=',lnew,'iw=',iw
                        write(io_output,*) 'CK here Torsional part NaN issue after cone...count=',count,'bendang=',bendang(count),'phi=',phi(count),'phidisp=',phidisp
                        write(io_output,*) 'CK here Torsional part NaN issue after cone...x=',x,'y=',y,'z=',z
                        write(io_output,*) 'CK here Torsional part NaN issue i=',i,'imolty=',imolty,orgmovetype
                        call err_exit(__FILE__,__LINE__,'Torsional part NaN issue after cone in rosenbluth',myid+1)
                   end if
                   ...
                   !write(io_output,*) 'CK here torsional count= ',count,' xx ',xx(count), ' yy ', yy(count),' zz ', zz(count)
                   if (lflex2rig.and.count.ne.1.and.iuprev.eq.riutry(imolty,1)) then
                                        iu = growlist(iw,count)
                                        ax1(1) = xx(1)
                                        ax1(2) = yy(1)
                                        ax1(3) = zz(1)
                                        !write(io_output,*) 'CK here b4 align count', count, ' iu ', iu, ' iw ', iw
                                        !write(io_output,*) 'CK here b4 align x', ax1(1), ' y ', ax1(2), ' z ',ax1(3)
                                        call align_planes_cbmc(iufrom,iuprev,growlist(iw,1),ax1,i,ax2(:,1),ax2(:,2),ax2(:,3),lnew)
                                        xx(count) = ax2(iu,1)
                                        yy(count) = ax2(iu,2)
                                        zz(count) = ax2(iu,3)
                   end if 
                ...
                do count = 1,ntogrow
                   iu = growlist(iw,count)
                   !write(io_output,*) 'CK chk torsion iu ',iu,' intor ',intor(imolty,iu)
                   if (lflex2rig.and.iuprev.ne.riutry(imolty,1).and.intor(imolty,iu).eq.0) then
                        cycle
                   end if
                   do it = 1, intor(imolty,iu)
                      ...
                      if ( jut2 .eq. iufrom .and.  jut3 .eq. iuprev) then
                         ...
                         !write(io_output,*) 'CK here in Rosen Torsion Front Part., count= ',count,' iu= ',iu,' it= ',it,'vdha=',vdha
                
                      end if
                   end do
                end do
300             continue
                ! CK softcut check here
                if (vdha * beta .gt. 2.3E0_dp*softcut) then ! CK softcut check
                  !write(io_output,*) 'caught by softcut',vdha*beta
                  lovr(my_itrial) = .true.
                  my_bf_tor(my_itrial) = 0.0E0_dp
                  my_vtorsion(my_itrial) = 0.0E0_dp
                else if ( vdha * beta .lt. -2.3E0_dp*softcut) then ! CK softcut check
                  !write(io_output,*) 'caught by softcut',vdha*beta
                  lovr(my_itrial) = .true.
                  my_bf_tor(my_itrial) = 0.0E0_dp
                  my_vtorsion(my_itrial) = 0.0E0_dp
                else if ( isnan(vdha) ) then
                  call err_exit(__FILE__,__LINE__,'NAN in torsion!!!!!!!!!',-1)
                ...
                end if !CK softcut check end
                !write(io_output,*) 'CK here in Rosen Torsion Middle Part vdha=',vdha
                ...
                !write(io_output,*) 'CK here in Rosen Torsion Part ip=',ip,'my_itrial',my_itrial,'bsum_tor(ip)',bsum_tor(ip)
             end do    ! end itor loop
          ...
          end if ! end of loop determine torsional angle
          ...
          do count = angstart,ntogrow
                ...
                if ( count .ne. 1 .and. lflex2rig .and. iuprev.eq.riutry(imolty,1)) then
                iu = growlist(iw,count)
                ax1(1) = xx(1)
                ax1(2) = yy(1)
                ax1(3) = zz(1)
                !write(io_output,*) 'CK here b4 align2 count', count, ' iu ', iu, ' iw ', iw
                !write(io_output,*) 'CK here b4 align2 x ', ax1(1), ' y ', ax1(2), ' z ',ax1(3)
                call align_planes_cbmc(iufrom,iuprev,growlist(iw,1),ax1,i,ax2(:,1),ax2(:,2),ax2(:,3),lnew)
                xx(count) = ax2(iu,1)
                yy(count) = ax2(iu,2)
                zz(count) = ax2(iu,3)
          ...
          !write(io_output,*) 'CK here after align2 checkpoint lflex2rig= ',lflex2rig,'iuprev= ',iuprev,' nunit ',nunit(imolty),' igrow ', igrow,' ntogrow ',ntogrow

          ! accepted coordinates, save them in r*p(trial)
          do count = 1,ntogrow
             if( .not. lflex2rig) then
                length = bondlen(count)
             else
                iu = growlist(iw,count)
                xvec(iu,iufrom)= rxu(i,iu) - rxu(i,iufrom)
                yvec(iu,iufrom)= ryu(i,iu) - ryu(i,iufrom)
                zvec(iu,iufrom)= rzu(i,iu) - rzu(i,iufrom)
                length = sqrt(xvec(iu,iufrom)**2 + yvec(iu,iufrom)**2 + zvec(iu,iufrom)**2)
             end if
       ...
       end do !End of generating itrial orientations
       !end if ! CK regular CBMC / grow rigid unit by rigrot end
250    continue
...
! ---------------------------------------------------------------------
       ...
       bsum = 0.0E0_dp
       !write(io_output,*)  'CK here first part. imolty= ',imolty,'original bsum=',bsum
       do ip = 1, ichoi
          ! include both the torsional and the LJ/qq
          bsum = bsum + bfac(ip)*bsum_tor(ip)
          if ( isnan(bfac(ip)).or.isnan(bsum_tor(ip)) ) then
                write(io_output,*) 'CK here detected nan issue.',ip," bsum= ",bsum," bfac= ",bfac(ip)," bsum_tor= ",bsum_tor(ip)
                write(io_output,*)  'CK here first part. i= ',i," imolty= ",imolty," movetype= ",orgmovetype
                call err_exit(__FILE__,__LINE__,'NaN issue in rosenbluth',myid+1)
          end if
       end do
       if ( lnew ) then
          ...
          rbf = bsum * random(-1)
          bs = 0.0E0_dp
          !write(io_output,*)  'CK here. imolty= ',imolty,"rbf= ",rbf
          do ip = 1, ichoi
             if ( .not. lovr(ip) ) then
                bs = bs + bfac(ip)*bsum_tor(ip)
                !write(io_output,*)  'CK here. ip= ',ip," bs= ",bs," bfac= ",bfac(ip)," bsum_tor= ",bsum_tor(ip)               
                if ( rbf .lt. bs ) then
                   ! select ip position ---
                   iwalk = ip
                   exit
       ...
       if ( lnew ) then
          ...
          !write(io_output,*) 'CK here. iwalk: ',iwalk,'imolty: ',imolty 
...
!*****************************************************************
  subroutine schedule(igrow,imolty,index,iutry,iprev,movetype,grouptype)
    ...
    integer :: gcbmc_imolty,unit_num_local,count_flex2rig
    real::dbgrow
    logical,parameter::lprint = .false. ! CK: remember to turn off this after modification...
! ------------------------------------------------------------------
#ifdef __DEBUG__
...
11  continue
    ...
    temp_count = 0
    lflex2rig = .false.
    if (lrigid(imolty).and.icbdir(imolty).eq.1) then
        lflex2rig = .true.
    end if
    ...
    if (movetype.eq.1) then
       ...
       if (lrigid(imolty).and.rindex(imolty).ne.1) then !CK modified
       ...
       else if (lrigid(imolty).and.rindex(imolty).eq.1.and..not.lflex2rig) then !CK modified for only one alkyl chain substuient on bezene ring
          random_index = 1
          !write(io_output,*) 'CK here, iutry_grow',riutry(imolty,random_index)
          iutry = int(riutry(imolty,random_index)*random(-1)) + 1
          !write(io_output,*) 'CK here, iutry',iutry
       else if (lflex2rig.and.icbsta(imolty).gt.0) then !CK modified for a partial rigid molecule to grow the flexible part first
          iutry = icbsta(imolty)
       else if (lflex2rig.and.icbsta(imolty).lt.0) then
          dbgrow = real(riutry(imolty,1)+icbsta(imolty)+1,dp)
          iutry = int(dbgrow*random(-1))-icbsta(imolty)
       else if (lflex2rig.and.icbsta(imolty).eq.0) then
          dbgrow = real(riutry(imolty,1)+1,dp)
       ...
       if (invtry.eq.0) then
          ...
          if (lrigid(imolty).and.idir.eq.1.and.iut.gt.riutry(imolty,1)+1.and.intor(imolty,iut).eq.0) then
             ! cannot grow from this bead, try again
             kickout = kickout + 1
             goto 11
          end if
          ...
          do ivib = 1,invtry
             ...
             if (grouptype.eq.1 .and. iut.eq.gcbmc_prev) then
                ...
                !write(io_output,*) 'CK here, temp_store(',temp_count,')=',temp_store(temp_count)
             end if
          end do
          do izz = temp_count,1,-1
             !write(io_output,*) 'CK here, izz',izz
             ! choose grow bead randomly from temp_store
50           itry = int(real(izz,dp)*random(-1)) + 1
             !write(io_output,*) 'CK here, itry',itry
             iut = temp_store(itry)
             if (lrigid(imolty).and.idir.eq.1.and.iut.gt.riutry(imolty,1)+1.and.intor(imolty,iut).eq.0.and.count.eq.0) then
                goto 50
             end if 
             !write(io_output,*) 'CK here, iut',iut
             count = count + 1
...
20              ivib = int(random(-1)*real(invtry,dp)) + 1 
                !write(io_output,*) 'CK here. Inside loop 20 , invtry= ',invtry,' ivib=  ',ivib
                iu = growlist(1,ivib)
                !write(io_output,*) 'CK here. Inside loop 20 , iu= ',iu
                ...
                if (lrigid(imolty).and.rindex(imolty).ne.1) then
                   if (iu.lt.riutry(imolty,1)) goto 20 ! Neeraj original: grow all flexible part entirely from the growth point
                else if (lrigid(imolty).and.rindex(imolty).eq.1.and..not.movetype.eq.6) then ! CK modified currently only for long alkyl chain on one site of a ring
                   ! need to create a situation that we need to regrow the other side, otherwise, infty loop in goto loop 70...
                   ! Numbering: First bead is next to the growth point
                   if (invib(imolty,1).gt.1) then
                        if(iutry.eq.riutry(imolty,1).and.iu.lt.riutry(imolty,1)) then
                                goto 20
                        else if (iutry.ne.riutry(imolty,1).and.iutry.ne.1.and.iu.gt.iutry) then
                                goto 20
                        else if (iutry.eq.1.and.iu.lt.riutry(imolty,1)) then
                                goto 20
                        else if (idir.eq.1.and.iu.gt.riutry(imolty,1)+1.and.intor(imolty,iu).eq.0.and.count_flex2rig.eq.0) then
                                goto 20
                        end if
                   else ! Numbering: First bead is at the end
                        if (iu.lt.iutry) goto 20
                   end if
                end if
                count_flex2rig = count_flex2rig + 1
                growprev(1) = iu
       ...
       !write(io_output,*) 'CK here scheduler type 3 iutry',iutry
       if (iutry.eq.0) then
       ...
       if (index.ne.0) then ! 
          ...
          do i=1,index
             ...
             growfrom(i)= riutry(imolty,i) ! CK comment out
             !growfrom(i)= gswatc ! CK gets growfrom from swatch parameters ! How do we get proper gswatc as the regrow site?
...
70     if ( outer_num .gt. 0 ) then
          ! CK Chang regrows the flex part first before grow a benzene ring
          !if (lrigid(imolty).and.iu .gt. riutry(imolty,1)  ) then
          !if (lrigid(imolty).and.icbdir(imolty).eq.1.and.iu .gt. riutry(imolty,1) ) then ! CK here for 2EP/4EP
          !   do iut = iu,nunit(imolty)
          !   lexshed(iut)=.false.
          !   end do
          !   index = index + 1 ! give the rigid unit an index...one more
          !   !iteration to calc torsion 
          !   growfrom(index) = iu
          !   growprev(index) = iufrom 
          !   grownum(index) = nunit(imolty) - iu
          !   outer_num = 0 ! break the goto loop 70
          !   goto 70
          !end if
          ...
          if ( lprint ) then
                write(io_output,*) 'CK here in CBMC schedule loop 70, outer_num', outer_num
                write(io_output,*) 'CK here in CBMC schedule loop 70, index', index
                write(io_output,*) 'CK here in CBMC schedule loop 70, iufrom', iufrom
                write(io_output,*) 'CK here in CBMC schedule loop 70, iu', iu
                write(io_output,*) 'CK here in CBMC schedule loop 70, outer_try', outer_try
          end if 
          ...
          count = 0
          do izz = temp_count, 1, -1
90           itry = int(real(izz,dp)*random(-1)) + 1
             iut = temp_store(itry)
             if (lrigid(imolty).and.idir.eq.1.and.iut.gt.riutry(imolty,1)+1.and.intor(imolty,iut).eq.0.and.count.eq.0) then
                goto 90                                                                                                                                         
             end if
...
!*********************************************************************
  subroutine geometry(lnew,iw,i,imolty,angstart,iuprev,glist,bondlen,bendang,phi,vvibtr,vbbtr,maxlen,wei_bend,lflex2rig)
    ...
    ntogrow = grownum(iw)
    iufrom = growfrom(iw)
    !write(io_output,*) 'CK here. iufrom in geom is',iufrom
    ! initialize trial energies
    vvibtr = 0.0E0_dp

    if ( .not. lnew .or. lflex2rig) then
    ...
    do count = 1,ntogrow
       ...
       do iv = 1, invib(imolty,iugrow)
          ...
          !write(io_output,*) 'CK here in geom. count',count,'iv',iv,'imolty',imolty,'iugrow',iugrow,'juvib',juvib
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
    ...
    ! if not use tabulated CBMC_bend_table, regular CBMC growth
        ...
        if (blocksize.gt.0) rcounts(1:blocksize) = rcounts(1:blocksize) + 1
            ...
            !if ( kforce .gt. 0.1E0_dp ) then !HF Hsiao-Feng: change condition to 0.0 to allow small kbend for separating M-site
            if ( kforce .gt. 0.0E0_dp ) then
                ! flexible bond angle
                ! initialize bsum_try
                bsum_try = 0.0E0_dp
                ...
                angle = acos(rsint)
                vangle=0.0E0_dp
            else if (lflex2rig .and. iufrom.eq.riutry(imolty,1)+1) then
                xvecgrow = rxu(i,iugrow) - rxui
                yvecgrow = ryu(i,iugrow) - ryui
                zvecgrow = rzu(i,iugrow) - rzui
                distgrow2 = xvecgrow **2 + yvecgrow**2 + zvecgrow**2
                distgrow = sqrt(distgrow2)
                ! dot product divided by lengths gives cos(angle)
                anglec = ( xvecprev*xvecgrow + yvecprev*yvecgrow  + zvecprev*zvecgrow ) / (distprev*distgrow)
                if(anglec.gt.1.0E0_dp) then
                           angle = 0.0E0_dp
...
!*****************************************************************
  subroutine cone(iinit,x,y,z,alpha,gamma)
    ...
    if ( iinit .eq. 1 ) then
       ...
       if (isnan(x).or.isnan(y).or.isnan(z)) then ! CK added for debugging
       write(io_output,*) 'CK in cone2..alpha',alpha,'sinalph',sinalph,'cosalph',cosalph
       write(io_output,*) 'CK in cone2..uztemp',uztemp,'uytemp',uytemp,'uztemp',uztemp
       write(io_output,*) 'CK in cone2..a11',a11,'a21',a21,'a31',a31
       write(io_output,*) 'CK in cone2..a12',a12,'a22',a22,'a32',a32
       write(io_output,*) 'CK in cone2..a13',a11,'a33',a33
       end if 
...
108    bond = (regrowWidth*random(-1)+minRegrow)
  ...
  subroutine align_planes_cbmc(bead1, bead2, bead3, ax,imol_b,xb,yb,zb,lnew)
    use const_math,only:onepi,twopi
    logical:: lnew
    ! INPUT VARIABLES ---
    integer:: imol_b, imol_a, itype_b, itype_a
    integer:: bead1, bead2, bead3
    real:: ax(3) ! position of the third bead determined by cbmc growth
    ! OUTPUT VARIABLES ---
...
#ifdef __DEBUG_VERBOSE__
    write(io_output,*) 'BEGIN align_planes_cbmc in ',myid
#endif
    imol_a = imol_b
    ...
    if(.not.lnew) then
        ...
        xorigin = rxnew(ia_bead1)
        yorigin = rynew(ia_bead1)
        zorigin = rznew(ia_bead1)
    end if 
    ...
    if(.not.lnew) then
        ...
        xa(2) = rxnew(ia_bead2) - xorigin
        ya(2) = rynew(ia_bead2) - yorigin
        za(2) = rznew(ia_bead2) - zorigin
    end if
    d = sqrt(xa(2)*xa(2) + ya(2)*ya(2) + za(2)*za(2))  
    xa(2) = xa(2)/d
    ya(2) = ya(2)/d
    za(2) = za(2)/d
    xa(3) = ax(1) 
    ...
    za(3) = ax(3) 
    !do i=1,3
    !    write(io_output,*) 'CK in align ',i,' pinpoint x ',xa(i),' y ',ya(i),' z ',za(i)
    !end do
    ! translate molecule be to the origin
    do i=riutry(imoltype_b,1),nunit_b
       xb(i) = rxu(imol_b,i) - rxu(imol_b,ib_bead1)
       yb(i) = ryu(imol_b,i) - ryu(imol_b,ib_bead1)
       zb(i) = rzu(imol_b,i) - rzu(imol_b,ib_bead1)
       d = sqrt(xb(i)*xb(i) + yb(i)*yb(i) + zb(i)*zb(i))
       if (d .gt. 1e-12) then
                xb(i) = xb(i)/d
                yb(i) = yb(i)/d
                zb(i) = zb(i)/d
       end if
       !write(io_output,*) 'CK in align translational ',i,' th bead x ',xb(i),' y ',yb(i),' z ',zb(i)
    end do
    ...
    if ( rnorm .gt. 1e-12 ) then ! CK for those who need to rotate
    xcross = xcross/rnorm
    ycross = ycross/rnorm
    zcross = zcross/rnorm
    ...
    if (isnan(xcross).or.isnan(ycross).or.isnan(zcross)) then
        write(io_output,*) 'align_plane, xcross',xcross,'ycross',ycross,'zcross',zcross,'rnorm',rnorm ! CK added for nan issue
    end if ! CK added for nan issue end
    do i=2,3
    ...
    ! CK checkpoint
    !do i=2,4
    !    write(io_output,*) 'CK align checkpoint in cbmc align. i= ',i,' xb ',xb(i),' yb ',yb(i),' zb ',zb(i)
    !end do

    end if ! CK added for those who need to rotate
    ...
    !write(io_output,*) 'The unit rotation axis rnorm= ',rnorm
    !if (rnorm .gt. 1e-12)  then ! CK added for those who need to rotate (second part)
    xcross = xa(2)
    ycross = ya(2)
    zcross = za(2)
    !write(io_output,*) 'xcross= ',xcross,' ycross= ',ycross,' zcross= ',zcross
    ! find projection of this unit vector onto yz plane
    d = sqrt(ycross*ycross + zcross*zcross)
    !write(io_output,*) 'd= ',d
    !write(io_output,*) 'CK is in second rotation xcross ', xcross,' ycross ', ycross,' zcross ',zcross,' d ',d
    if ( d .gt. 1e-12 ) then
    do i=2,3
       ytemp = (ya(i)*zcross - za(i)*ycross)/d
       ztemp = (ya(i)*ycross + za(i)*zcross)/d
    ...
    do i=riutry(imoltype_b,1),nunit_b
       ...
       xb(i) = xtemp
       zb(i) = ztemp
       !write(io_output,*) 'CK is here. i= ',i,' xb ',xb(i),' yb ',yb(i),' zb ', zb(i)
    end do
    else
        call err_exit(__FILE__,__LINE__,'The original plane is on xy plane. Rotate init struct slightly or modify the code.',-1)
    end if
    !CC -- STEP 4, Rotate by theta that about z-axis
    ...
    !end if  ! CK added for those who need to align (second part)
    !do i=riutry(imoltype_b,1),nunit_b
    !   write(io_output,*) 'O ',xb(i),yb(i),zb(i)
    !end do
...
! Write a check
    ...
    ! end do
    ! close(90)
    write(io_output,*) 'END align_planes_cbmc ',myid
#endif
  end subroutine align_planes_cbmc      
  ...
  subroutine align_lines(first_bead, second_bead, nbead, xi, yi, zi, xtarget, ytarget, ztarget)
      ...
      if(isnan(xtrans).or.isnan(ytrans).or.isnan(ztrans)) write(io_output,*) 'CK found nan!!! Translational part.'
      do ibead = 1, nbead
          xi(ibead) = xi(ibead) + xtrans
          yi(ibead) = yi(ibead) + ytrans
          zi(ibead) = zi(ibead) + ztrans
          if(isnan(xi(ibead)).or.isnan(yi(ibead)).or.isnan(zi(ibead))) then
                write(io_output,*) 'CK found nan!!! Translational part. DO loop. ibad',ibead
          end if
      end do
      ...
      if(isnan(vector_a(1)).or.isnan(vector_a(2)).or.isnan(vector_a(3))) then
            write(io_output,*) 'CK found vector_a is NAN!!!'
            write(io_output,*) 'x',vector_a(1),'y',vector_a(2),'z',vector_a(3)
      end if
      if(isnan(vector_b(1)).or.isnan(vector_b(2)).or.isnan(vector_b(3))) then
            write(io_output,*) 'CK found vector_b is NAN!!!'
            write(io_output,*) 'x',vector_b(1),'y',vector_b(2),'z',vector_b(3)
      end if
      ...
      norm_temp = sqrt (vector_cross(1) * vector_cross(1) + vector_cross(2) * vector_cross(2) + vector_cross(3) * vector_cross(3))
      if (norm_temp .gt. 1e-12) then ! CK added to prevent from NAN issue
      ...
      end if ! CK added to prevent from NAN issue end  
      ...
      do ibead = 1, nbead
          ...
          xi(ibead) = xi(ibead) + vector_rot(1) ! CK changed the index here
          yi(ibead) = yi(ibead) + vector_rot(2) ! CK changed the index here
          zi(ibead) = zi(ibead) + vector_rot(3) ! CK changed the index here
          write(io_output,*) 'CK here in align_lines. ibead',ibead,'xi',xi(ibead),'yi',yi(ibead),'zi',zi(ibead)
          write(io_output,*) 'CK here in align_lines. first_bead',first_bead,'xi',xi(first_bead),'yi',yi(first_bead),'zi',zi(first_bead)
      end do
      ! make sure it matches
      bead_err = abs((xi(second_bead) - xtarget(2))**2 + (yi(second_bead) - ytarget(2))**2 + (zi(second_bead) - ztarget(2))**2)
      if (bead_err .gt. 1e-4) then ! CK turned off
         call err_exit(__FILE__,__LINE__,'error in align_lines subroutine, the second bead does not match with the target',-1)
      end if ! CK turned off end
  end subroutine align_lines
  
  subroutine align_lines_partial_rigid_cbmc(imol_b,itype_b,xb,yb,zb)
    use const_math,only:onepi,twopi

    ! INPUT VARIABLES ---
    integer:: imol_b,  itype_b
...
#ifdef __DEBUG_VERBOSE__
    write(io_output,*) 'BEGIN align_lines_partial_rigid cbmc in ',myid
#endif

    imoltype_b = moltyp(imol_b)
    nunit_b = nunit(imoltype_b)

    ib_bead1=riutry(imoltype_b,1) + 1
    ib_bead2=riutry(imoltype_b,1)
    ib_bead3=riutry(imoltype_b,1) - 1
    ...
    ! save position of bead_1 on molecule_a
    xorigin = rxflex(ib_bead1)
    yorigin = ryflex(ib_bead1)
    zorigin = rzflex(ib_bead1)
    !write(io_output,*) 'CK inside align origin',xorigin,yorigin,zorigin
    ! use this bead as the origin
    xa(1) = 0.0E0_dp
    ya(1) = 0.0E0_dp
    za(1) = 0.0E0_dp
    xa(2) = ( rxflex(ib_bead2) - xorigin )
    ya(2) = ( ryflex(ib_bead2) - yorigin )
    za(2) = ( rzflex(ib_bead2) - zorigin )
    xa(3) = rxflex(ib_bead3) - xorigin
    ya(3) = ryflex(ib_bead3) - yorigin
    za(3) = rzflex(ib_bead3) - zorigin

    ! translate molecule be to the origin
    do i=1,nunit_b
       xb(i) = xb(i) - rxu(imol_b,ib_bead1)
       yb(i) = yb(i) - ryu(imol_b,ib_bead1)
       zb(i) = zb(i) - rzu(imol_b,ib_bead1)
    end do
    ...
    ! normalize cross product to unit length
    rnorm = sqrt(xcross*xcross + ycross*ycross + zcross*zcross)
    !write(io_output,*) 'CK here norm=',rnorm
    if ( rnorm .gt. 1e-12 ) then ! CK for those who need to rotate
    xcross = xcross/rnorm
    ycross = ycross/rnorm
    zcross = zcross/rnorm
    ...
    do i=1,nunit_b
       ...
       yb(i) = yb(i) + yorigin
       zb(i) = zb(i) + zorigin
       !write(io_output,*) 'CK inside align...',xb(i),yb(i),zb(i)
    end do
    end if ! CK added for those who need to rotate
    !CC --- FINALLY WE ARE DONE
#ifdef __DEBUG_VERBOSE__
    write(io_output,*) 'END align_lines_partial_rigid',myid
#endif
  end subroutine align_lines_partial_rigid_cbmc
  ...
  subroutine dihedral_rigrot(bead1_coord, bead2_coord, nbead, xi, yi, zi, phi, xout, yout, zout, imolty, mvtyp)
      use util_runtime, only : err_exit
      ...
      if (mvtyp .eq. 2) then
         do ibead = nbead, riutry(imolty,1)+1, -1 ! CK: be aware of the value of nbead
          ...
          !write(io_output,*) 'CK inside dihedral_rigrot...ibead',ibead,'xout',xout(ibead),'yout',yout(ibead),'zout',zout(ibead)
        end do
      else ! original setup
        ! loop over all unit and grow
        do ibead = 1, nbead ! CK: be aware of the value of nbead
          vector_grow(1) = xout(ibead) - bead2_coord(1)
          vector_grow(2) = yout(ibead) - bead2_coord(2)
          vector_grow(3) = zout(ibead) - bead2_coord(3)
...
!************************************************************
  subroutine rigrot(lnew,lterm,iskip,imol,imolty,ibox,wadd,first_bead,second_bead,mvtp)
    ...
    integer::ibox,igrow,j,ip,iwalk,iunit,imolty,iu,iskip,igrow2,it
    ...
    real,allocatable::vdha(:),bfdha(:)
    real::xvec1,yvec1,zvec1,xvec2,yvec2,zvec2,xvec3,yvec3,zvec3 !CK added for torsional calc for growing 2EP/4EP
! ----------------------------------------------------------------------
#ifdef __DEBUG__
...
! initialize conformation energies and weight
    ...
    ltors = .false.
    iunit = nunit(imolty)
    allocate (vdha(ichoi),bfdha(ichoi))
    if(mvtp.eq.0) then  !BX
       igrow = first_bead
       igrow2 = 0
    else if(mvtp.eq.5.or.mvtp.eq.6) then ! CK modified for flex/partial rigid swatch
       igrow = second_bead
       igrow2 = first_bead
    else
    ...
    else if (mvtp.eq.6) then !CK: this part is for flex-[partial rigid] swatch
       !write(io_output,*) 'CK is using this part... rigrot for flex-[partial rigid] swatch'
       do ip=1,ichoi
          if(lnew) then
             count = 1
             do j=1,iunit
                ...
                !write(io_output,*) 'CK here in rigrot typ6...count',count,'ip',ip,'j',j
                !write(io_output,*) 'CK here in rigrot,rxnew(j)',rxnew(j),'rynew(j)',rynew(j),'rznew(j)',rznew(j)
                !write(io_output,*) 'rxp',rxp(count,ip),'ryp',ryp(count,ip),'rzp',rzp(count,ip)
                count = count + 1
             end do
             ax1(1) = rxnew(second_bead)
             ax1(2) = rynew(second_bead)
             ax1(3) = rznew(second_bead)
             ax2(1) = rxnew(first_bead)
             ax2(2) = rynew(first_bead)
             ax2(3) = rznew(first_bead)
             !write(io_output,*) 'CK here in rigrot typ6...ax1',ax1,'ax2',ax2
          else
             count = 1
             do j=1,iunit
                if(j.eq.first_bead.or.j.eq.second_bead) cycle
             ...
             ax1(1) = rxu(imol,second_bead)
             ax1(2) = ryu(imol,second_bead)
             ax1(3) = rzu(imol,second_bead)
             ax2(1) = rxu(imol,first_bead)
             ax2(2) = ryu(imol,first_bead)
             ax2(3) = rzu(imol,first_bead)
          end if

          if(lnew.or.ip.ne.1) then
             phi = twopi*random(-1)
             !unit to grow: tot units - riutry - 1
             !call dihedral_rigrot(ax1,ax2,iunit-riutry(imolty,1)-1,rxp(:,ip),ryp(:,ip),rzp(:,ip),phi,rxp(:,ip),ryp(:,ip),rzp(:,ip),imolty,2)
             call dihedral_rigrot(ax1,ax2,iunit,rxp(:,ip),ryp(:,ip),rzp(:,ip),phi,rxp(:,ip),ryp(:,ip),rzp(:,ip),imolty,2)
          end if
       end do
    else if (mvtp.eq.7) then !CK: this part is for 2EP/4EP growing the flexible part first.
       do ip=1,ichoi
          if(lnew) then
             ax1(1) = rxnew(second_bead)
             ax1(2) = rynew(second_bead)
             ...
             do j=1,icbsta(imolty)
                rxflex(j) = rxu(imol,j)
                ryflex(j) = ryu(imol,j)
                rzflex(j) = rzu(imol,j)

             end do
             do j=icbsta(imolty)+1,igrow
                rxflex(j) = rxnew(j)
                ryflex(j) = rynew(j)
                rzflex(j) = rznew(j)
             end do
             ...
             call align_lines_partial_rigid_cbmc(imol,imolty,rxnew,rynew,rznew)
             count = 1
              do j=1,igrow
                rxp(count,ip) = rxflex(j)
                ryp(count,ip) = ryflex(j)
                rzp(count,ip) = rzflex(j)
                count = count + 1
             end do
             do j=igrow+1,iunit
                rxp(count,ip) = rxnew(j)
    ...
    do j=1,iunit
       lexist(j) = .false.
       if(j.eq.igrow.or.(mvtp.eq.5.and.j.eq.igrow2).or.( ( mvtp.eq.6 .or. mvtp.eq.7 ).and.j.le.riutry(imolty,1)+1)) then ! CK modified for flex/partial rigid swatch
          lexist(j) = .true.
          !write(io_output,*) 'CK is here.. inside boltz j, lexist(j)',j,lexist(j)
          cycle
       end if
       !write(io_output,*) 'CK is here.. inside boltz j, lexist(j)',j,lexist(j)
    end do
    
    if(mvtp.eq.6 .or. mvtp.eq.7) then ! CK modified for flex/partial rigid swatch if start
        xvec1 = rxp(riutry(imolty,1),1) - rxp(riutry(imolty,1)-1,1)
        yvec1 = ryp(riutry(imolty,1),1) - ryp(riutry(imolty,1)-1,1)
        zvec1 = rzp(riutry(imolty,1),1) - rzp(riutry(imolty,1)-1,1)
        xvec2 = rxp(riutry(imolty,1)+1,1) - rxp(riutry(imolty,1),1)
        yvec2 = ryp(riutry(imolty,1)+1,1) - ryp(riutry(imolty,1),1)
        zvec2 = rzp(riutry(imolty,1)+1,1) - rzp(riutry(imolty,1),1)
        do ip = 1, ichoi
                bfdha(ip) = 1.0d0
                vdha(ip) = 0.0d0
                do it = 1, intor(imolty,riutry(imolty,1)-1)
                        if (ijtor2(imolty,riutry(imolty,1)-1,it) .lt. (riutry(imolty,1)-1) ) cycle

                        xvec3 = rxp(ijtor4(imolty,riutry(imolty,1)-1,it),ip) - rxp(riutry(imolty,1)+1,ip)
                        yvec3 = ryp(ijtor4(imolty,riutry(imolty,1)-1,it),ip) - ryp(riutry(imolty,1)+1,ip)
                        zvec3 = rzp(ijtor4(imolty,riutry(imolty,1)-1,it),ip) - rzp(riutry(imolty,1)+1,ip)
                        vdha(ip) = vdha(ip) + vtorso(xvec1,yvec1,zvec1,xvec2,yvec2,zvec2,xvec3,yvec3,zvec3,ittor(imolty,riutry(imolty,1)-1,it))
                end do
                bfdha(ip) = exp ( -( vdha(ip) * beta) )
                !write(io_output,*) 'CK is here
                !ip',ip,'vdha',vdha(ip),'bfdha',bfdha(ip)
        end do
        !ntogrow = iunit-igrow
    else
        do j=igrow+1, iunit
    ...
    end if ! CK modified for flex/partial rigid swatch if end
    !if (ovrlap) then
    !   lterm = .true.
    !   return
    !end if
    ...
    bsum = 0.0E0_dp
    if(mvtp.eq.6.or.mvtp.eq.7) then ! CK added for mvtp 6 or mvtp 7
    do ip = 1, ichoi
       bsum = bsum + bfdha(ip)
    end do
    ...
    end if !CK added for mvtp 6 end 
    wadd = wadd * bsum
    ...
    if (lnew) then
       ...
       do ip = 1, ichoi
          if (.not. lovr(ip) ) then
             bs = bs + bfac(ip)
             !write(io_output,*) 'CK here in rigrot. ip',ip,'bfac(ip)',bfac(ip),'bs',bs
             if ( rbf .lt. bs ) then
                ! select ip position
                iwalk = ip
                goto 15
...
15     continue
    ...
    if (.not.(mvtp.eq.6.or.mvtp.eq.7)) then !CK exclude mvtp 6 and 7
            if ( lnew ) then
...
! now we must add up energies and record new positions
  ...
  subroutine group_cbmc_grow(lnew,lterm,i,imolty,ibox)
      ...
      do while (.true.)
         ...
         ! determine which segment to grow (i_grow_unit) ! CK: This is smt like
         ! goto loop 20 in regular cbmc scheduler.
...
925      continue
         ...
         repeat_unit_nunit = nugrow(repeat_unit_imolty) ! CK: unit number of the selected segment
...
! -------------------------------------------------------------------
    ...
    call mp_bcast(probf,30*30*maxbin,rootid,groupid) ! Paul  !< CK Chang changed from 60 to 30
    hist = 0._dp
