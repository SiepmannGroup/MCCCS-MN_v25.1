...
module transfer_swatch
  ...
  use energy_pairwise,only:energy,coru,energyjc,calrhoz2,energyjc2,calrhoz4,jccoru
  use moves_cbmc,only:rosenbluth,schedule,safeschedule,explct,lexshed,align_lines,group_cbmc_grow
  implicit none
  private
  save
...
!> \since 9-25-02 JMS
  subroutine swatch()
    use sim_particle,only:ctrmas,update_coord_in_tree,restore_neighbor_list,save_neighbor_list&
        ,update_neighbor_list_molecule_swatch,update_neighbor_list_molecule_swatch_v2,neigh_icnt_v2,neighbor
    ...
    real::rpair,tweight,tweiold,vnbox(nEnergy,nbxmax),rx_1(numax),ry_1(numax),rz_1(numax),rxut(4,numax),ryut(4,numax),rzut(4,numax)&
     ...
     ,length_current,length_modified !< CK uses for normalization
    real::vTailn,vTailo,Vtaila,vTailb
    ...
    integer::iii,j
    integer::oldchain,newchain,oldunit,newunit,iins,aunit,bunit
    ...
    oldnew: do ic = 1,2
       ...
       if (ncut(iparty,s_type) .gt. 0) then
          ...
          if (lrigid(imolty)) then
             ...
             if (rindex(imolta).eq.0.and.rindex(imoltb).eq.0) then ! CK keeps the treatments for PAH rigid--rigid
          ...
          !end if
             else if (rindex(imolta).eq.1.and.rindex(imoltb).eq.1.and.nsampos(iparty).gt.3) then ! CK added case for partial rigid molecules with a flexible chain on each
                ! CK impletment only for one growth point on each molecule
                ! nsampos .gt. 3 to exclude phenol--2,5dimethylphenol case
                !> \bug problem if nsampos.eq.iunit, but rindex.gt.0
                !call schedule(igrow,imolty,islen,ifirst,iprev,4,0) !CK comment out Jake original
                call schedule(igrow,imolty,islen,ifirst,iprev,3,0)  !CK: flexible--flexible or partial rigid--partial rigid
             !cc---END JLR 11-24-09
             else if ((lrigid(imolta).and..not.lrigid(imoltb)).or.(lrigid(imoltb).and..not.lrigid(imolta))) then 
                ! CK: remember to change the bond length of flexible molecule in
                ! flex--[partial rigid] swatch
                call align_lines_partial_rigid(iparty,self,other,s_type,o_type,rxnew,rynew,rznew)
                !call align_planes(iparty,self,other,s_type,o_type,rxnew,rynew,rznew)
                do icbu = 2, nsampos(iparty) ! restore the beads that detemined by alcohols
                        new = splist(iparty,icbu,s_type)
                        old = splist(iparty,icbu,o_type)
                        rxnew(new) = rxu(other,old)
                        rynew(new) = ryu(other,old)
                ...
                !if(.false.) then
                !        write(io_output,*) 'CK here write out the geometry..'
                !        do j = 1,iunit
                !        write(io_output,*) rxnew(j),rynew(j),rznew(j)
                !        end do
                !end if
                call schedule(igrow,imolty,islen,ifirst,iprev,6,0)
             else ! CK added case for partial rigid--flexible (e.g. benzene--phenol)
                ! CK: align first, then regrow...
                !write(io_output,*) 'CK is here. using else in swatch!!!'
                call align_planes(iparty,self,other,s_type,o_type,rxnew,rynew,rznew)     
                call schedule(igrow,imolty,islen,ifirst,iprev,4,0) 
             end if ! CK keeps the treatments for PAH end
          else
             if ((lrigid(imolta).and..not.lrigid(imoltb)).or.(lrigid(imoltb).and..not.lrigid(imolta))) then
                ! CK: remember to change the bond length of flexible molecule in
                ! [flex]--rigid swatch
                first_bead = splist(iparty,2,s_type)
                second_bead = splist(iparty,1,s_type) ! the bead need to dilation
                length_current = sqrt((rxnew(second_bead) - rxnew(first_bead))**2 + (rynew(second_bead) - rynew(first_bead))**2 &
                                        + (rznew(second_bead) - rznew(first_bead))**2)
                length_modified = sqrt((rxu(self,second_bead) - rxu(self,first_bead))**2 + (ryu(self,second_bead) - ryu(self,first_bead))**2 &
                                        + (rzu(self,second_bead) - rzu(self,first_bead))**2)
                rxnew(second_bead) = rxnew(first_bead) + ( rxnew(second_bead) - rxnew(first_bead) ) * length_modified / length_current
                rynew(second_bead) = rynew(first_bead) + ( rynew(second_bead) - rynew(first_bead) ) * length_modified / length_current
                rznew(second_bead) = rznew(first_bead) + ( rznew(second_bead) - rznew(first_bead) ) * length_modified / length_current
             end if
             call schedule(igrow,imolty,islen,ifirst,iprev,3,0)  !CK: For regular swatch moves such as alkanes/alcohols
          end if
          ...
          ! CK Chang do dihedral rigrot to generate trail 
!******************
! CBMC - new growth
!******************
          if (boxa.eq.boxb) then
          ...
          if (lrigid(imolty)) then
             if (nsampos(iparty).ge.iunit) then
                ...
                icallrose = 5
             else if ((lrigid(imolta).and..not.lrigid(imoltb)).or.(lrigid(imoltb).and..not.lrigid(imolta))) then !CK
                icallrose = 6
          ...
          if (boxa.eq.boxb) then
                ...
                !if(.not.((.not.lrigid(imolty)).and.((lrigid(imolta).and..not.lrigid(imoltb)).or.(lrigid(imoltb).and..not.lrigid(imolta))))) then
                nboxi(other) = 0 ! CK added
                call rosenbluth(.true.,lterm,other,self,imolty,islen,iboxnew,igrow,waddnew,.false.,vdum2,icallrose,first_bead,second_bead)
                nboxi(other) = iboxnew ! CK added
                !else ! If [flex]-partial rigid swach, do not calc. rosen. for flex molecules
                !lterm=.false.
                !end if
         ...
         rzu(self,izz) = rzut(ic,izz)
          ...
          if (ncutsafe(iparty,s_type) .eq. 0) then
             ...
             if (boxa.eq.boxb) then
                ...
                !write(io_output,*) 'other=',other,'iboxnew=',iboxnew !HF
             end if
...
1000      waddold = 1.0E0_dp
          ...
          if (lrigid(imolty)) then
             if (nsampos(iparty).ge.iunit) then
                ...
                !> \bug why don't do rigrot if ifirst.ge.riutry (ifirst is part of rigid beads)?
                ! else if ((nsampos(iparty).ge.3).or.(rindex(imolty).gt.0.and.ifirst.ge.riutry(imolty,1))) then
             else if ((lrigid(imolta).and..not.lrigid(imoltb)).or.(lrigid(imoltb).and..not.lrigid(imolta))) then ! CK: changes the logic here...
                ! for flex-[partial rigid] swatch
                icallrose = 6
             else if (nsampos(iparty).ge.3) then
                ! rigid part is grown, don't do rigrot in rosebluth
                icallrose = 3
          ...
          if (lrigid(imolty).and.(lrigid(imolta).and..not.lrigid(imoltb)).or.(lrigid(imoltb).and..not.lrigid(imolta))) then ! CK modified for MeOH-Phenol swatch
                call rosenbluth(.false.,lterm,self,self,imolty,islen,iboxold,igrow,waddold,.false.,vdum2,icallrose,first_bead,second_bead) 
          else
                !if(.not.((lrigid(imolta).and..not.lrigid(imoltb)).or.(lrigid(imoltb).and..not.lrigid(imolta)))) then
                 call rosenbluth(.false.,lterm,self,self,imolty,islen,iboxold,igrow,waddold,.false.,vdum2,icallrose,first_bead,second_bead)
                !else
                ! lterm=.false.
                !end if
      ...
      call U_bonded(self,imolty,tmpvvib,tmpvbend,tmpvtg)
          ...
          if (ncutsafe(iparty,s_type) .eq. 0) then
             ...
             !write(io_output,*) 'self=',self,'iboxold=',iboxold!HF
...
!*******************************
       ...
       do izz = 1, ncutsafe(iparty,s_type)
          ...
          if (boxa.eq.boxb) then ! CK didn't implement flexible-partial rigid (e.g. MeOH/phenol) for SAFE-CBMC
    ...
    if ((ltailc.and..not.ljc(boxa).and..not.lideal(boxa)).and.(ltailc.and..not.ljc(boxb).and..not.lideal(boxb)).and.boxa.ne.boxb) then
   ...
   if(boxa .eq. boxb .and. ljc(boxa)) then
      ! molty a
      vTaila = 0.0_dp
      aunit = nunit(imolta)
      do j=1,aunit
         rxuion(j,1) = rxu(imola,j)
         ryuion(j,1) = ryu(imola,j)
         rzuion(j,1) = rzu(imola,j)
         rxuion(j,2) = rxut(1,j)
         ryuion(j,2) = ryut(1,j)
         rzuion(j,2) = rzut(1,j)
      end do

      call energyjc(1,aunit,vTailo,imolta,boxa)
      call calrhoz2(imolta,boxa,2)
      call calrhoz2(imolta,boxa,1)
      call energyjc(1,aunit,vTailn,imolta,boxa)

      vTaila = vTailn - vTailo

      ! molty b
       bunit = nunit(imoltb)
      do j=1,bunit
         rxuion(j,1) = rxu(imolb,j)
         ryuion(j,1) = ryu(imolb,j)
         rzuion(j,1) = rzu(imolb,j)
         rxuion(j,2) = rxut(2,j)
         ryuion(j,2) = ryut(2,j)
         rzuion(j,2) = rzut(2,j)
      end do

      call energyjc(1,bunit,vTailo,imoltb,boxa)
      call calrhoz2(imoltb,boxa,2)
      call calrhoz2(imoltb,boxa,1)
      call energyjc(1,bunit,vTailn,imoltb,boxa)
      vTaila = vTailn - vTailo + vTaila
      tweight = tweight * exp(-beta*vTaila)
      vnbox(ivTail,boxa)= vTaila
      vnbox(ivTail,boxb) = vTaila
   else
       ! boxa
      if(ltailc.and..not.ljc(boxa).and..not.lideal(boxa)) then
         vTaila = 0.0_dp
         if (lpbcz) then
             if (lsolid(boxa) .and. .not. lrect(boxa)) then
                 vola = (hmat(boxa,1) * (hmat(boxa,5) * hmat(boxa,9) - hmat(boxa,8)*hmat(boxa,6))+ hmat(boxa,4)*(hmat(boxa,8) *hmat(boxa,3)-hmat(boxa,2)* hmat(boxa,9))+hmat(boxa,7) * (hmat(boxa,2)*hmat(boxa,6)- hmat(boxa,5)*hmat(boxa,3)))
         ...
         do jmt = 1, nmolty
            ...
            rho = rho / dvol
             do imt = 1, nmolty
                ...
                if ( imt .eq. imolin ) dicount = dicount + 1
                if ( imt .eq. imolrm ) dicount = dicount - 1
                vTaila = vTaila +  dicount * coru(imt,jmt,rho,ibox)
             end do
            end do
         vTaila = vTaila - vbox(ivTail,ibox)
         tweight=tweight*exp(-beta*vTaila)
         vnbox(ivTail,ibox) = vTaila
      end if
      ...
      if(ljc(boxa)) then
         ...
         call calrhoz4(.FALSE.,imolta,boxa,1)
         call calrhoz4(.TRUE.,imoltb,boxa,1)
         call energyjc2(.FALSE.,1,aunit,vTailo,imolta,boxa)
         call calrhoz4(.FALSE.,imolta,boxa,2)
         call energyjc2(.FALSE.,1,aunit,vTailn,imolta,boxa)
         vTaila = vTailn - vTailo
         !add imoltb
         call energyjc2(.TRUE.,1,bunit,vTailo,imoltb,boxa)
         call calrhoz4(.TRUE.,imoltb,boxa,2)
         call energyjc2(.TRUE.,1,bunit,vTailn,imoltb,boxa)
         vTaila = vTailn - vTailo + vTaila
         vnbox(ivTail,boxa) = vTaila
         tweight=tweight*exp(-beta*vTaila)
      end if
      ...
      if(ljc(boxb)) then
         ...
         call calrhoz4(.FALSE.,imoltb,boxb,1)
         call calrhoz4(.TRUE.,imolta,boxb,1)
         call energyjc2(.FALSE.,1,bunit,vTailo,imoltb,boxb)
         call calrhoz4(.FALSE.,imoltb,boxb,2)
         call energyjc2(.FALSE.,1,bunit,vTailn,imoltb,boxb)
         vTailb = vTailn - vTailo
         !add imolta
         call energyjc2(.TRUE.,1,aunit,vTailo,imolta,boxb)
         call calrhoz4(.TRUE.,imolta,boxb,2)
         call energyjc2(.TRUE.,1,aunit,vTailn,imolta,boxb)
         vTailb = vTailn - vTailo + vTailb
         vnbox(ivTail,boxb) = vTailb
         tweight=tweight*exp(-beta*vTailb)
      end if

      if(ltailc.and..not.ljc(boxb).and..not.lideal(boxb)) then
         vTailb = 0.0_dp
         if (lpbcz) then
             if (lsolid(boxb) .and. .not. lrect(boxb)) then
                 volb = (hmat(boxb,1) * (hmat(boxb,5) * hmat(boxb,9) -hmat(boxb,8)*hmat(boxb,6))+ hmat(boxb,4)*(hmat(boxb,8)*hmat(boxb,3)-hmat(boxb,2)* hmat(boxb,9))+hmat(boxb,7) *(hmat(boxb,2)*hmat(boxb,6)- hmat(boxb,5)*hmat(boxb,3)))
         ...
         do jmt = 1, nmolty
            ...
            rho = rho / dvol
             do imt = 1, nmolty
                ...
                if ( imt .eq. imolin ) dicount = dicount + 1
                if ( imt .eq. imolrm ) dicount = dicount - 1
                vTailb = vTailb +  dicount * coru(imt,jmt,rho,ibox)
             end do
         end do
         vTailb = vTailb - vbox(ivTail,ibox)
         tweight=tweight*exp(-beta*vTailb)
         vnbox(ivTail,ibox) = vTailb
      end if
    end if !!CCK debug
    end if
    wnlog = log10( tweight )
    ...
    if ( wdlog .lt. -softcut ) then
       ...
       if(ljc(boxa).and. boxa .eq. boxb) then
          do j=1,aunit
             rxuion(j,1) = rxu(imola,j)
             ryuion(j,1) = ryu(imola,j)
             rzuion(j,1) = rzu(imola,j)
         ...
         call calrhoz2(imoltb,boxb,3)
          ...
          if(ljc(boxa)) then
             ...
             call calrhoz4(.FALSE.,imolta,boxa,3)
             call calrhoz4(.TRUE.,imoltb,boxa,3)
          end if
          if(ljc(boxb)) then
             aunit = nunit(imolta)
             bunit = nunit(imoltb)
             ...
             call calrhoz4(.FALSE.,imoltb,boxb,3)
             call calrhoz4(.TRUE.,imolta,boxb,3)
          end if
     end if
       return
    end if
...
!          write(*,*) 'total tor diff', total_tor
       ...
       if (lneigh.or.lneighbor.or.lgaro) then ! CK added for swatch
           !call update_neighbor_list_molecule_swatch(imola,imolb) !CK
           !write(io_output,*) 'Before update:'
           !write(io_output,*) 'neigh_icnt_v2(',imola,')=',neigh_icnt_v2(imola),',neigh_icnt_v2(',imolb,')=',neigh_icnt_v2(imolb)
           !write(io_output,*) 'neighbor(',imola,')=',neighbor(1:neigh_icnt_v2(imola)+2,imola)
           !write(io_output,*) 'neighbor(',imolb,')=',neighbor(1:neigh_icnt_v2(imolb)+2,imolb)
           !write(io_output,*)  'imola=',imola,'molty(a)=',moltyp(imola),'nboxi(a)=',nboxi(imola),&
           !        'imolb=',imolb,'molty(b)=',moltyp(imolb),'nboxi(b)=',nboxi(imolb)
           !write(io_output,*) 'other=',other
           call update_neighbor_list_molecule_swatch_v2(imola,imolb) !HF
           !write(io_output,*) 'swatch pair=', imola, imolb!HF
           !write(io_output,*) 'After update:'
           !write(io_output,*) 'neigh_icnt_v2(',imola,')=',neigh_icnt_v2(imola),',neigh_icnt_v2(',imolb,')=',neigh_icnt_v2(imolb)
           !write(io_output,*) 'neighbor(',imola,')=',neighbor(1:neigh_icnt_v2(imola)+2,imola)
           !write(io_output,*) 'neighbor(',imolb,')=',neighbor(1:neigh_icnt_v2(imolb)+2,imolb)
       end if !neighbor end
...
!>
!> 1) Translates so swatch bead 1 of the swatched molecule is in
...
! Write a check
  ...
  subroutine align_lines_partial_rigid(isplist,imol_b,imol_a,itype_b,itype_a,xb,yb,zb)
    use const_math,only:onepi,twopi
    ...
    real::length_current, length_modified
    real::xa(3),   ya(3),   za(3)
    real::xorigin, yorigin, zorigin
    real::xcross,  ycross,  zcross
    real::xtemp,   ytemp,   ztemp
...
#ifdef __DEBUG_VERBOSE__
    write(io_output,*) 'BEGIN align_lines_partial_rigid in ',myid
#endif
    ...
    ! save position of bead_1 on molecule_a
    ! origin needs dilation
    length_current = sqrt((rxu(imol_a,ia_bead1) - rxu(imol_a,ia_bead2))**2 + (ryu(imol_a,ia_bead1) - ryu(imol_a,ia_bead2))**2 &
                                        + (rzu(imol_a,ia_bead1) - rzu(imol_a,ia_bead2))**2)
    length_modified = sqrt((rxu(imol_b,ib_bead1) - rxu(imol_b,ib_bead2))**2 + (ryu(imol_b,ib_bead1) - ryu(imol_b,ib_bead2))**2 &
                                        + (rzu(imol_b,ib_bead1) - rzu(imol_b,ib_bead2))**2)   
    xorigin = rxu(imol_a,ia_bead2) + ( rxu(imol_a,ia_bead1) - rxu(imol_a,ia_bead2) ) * length_modified / length_current
    yorigin = ryu(imol_a,ia_bead2) + ( ryu(imol_a,ia_bead1) - ryu(imol_a,ia_bead2) ) * length_modified / length_current
    zorigin = rzu(imol_a,ia_bead2) + ( rzu(imol_a,ia_bead1) - rzu(imol_a,ia_bead2) ) * length_modified / length_current
    ! use this bead as the origin
    xa(1) = 0.0E0_dp
    ya(1) = 0.0E0_dp
    za(1) = 0.0E0_dp
    xa(2) = ( rxu(imol_a,ia_bead2) - xorigin ) * length_modified / length_current
    ya(2) = ( ryu(imol_a,ia_bead2) - yorigin ) * length_modified / length_current 
    za(2) = ( rzu(imol_a,ia_bead2) - zorigin ) * length_modified / length_current 
    xa(3) = rxu(imol_a,ia_bead3) - xorigin
    ya(3) = ryu(imol_a,ia_bead3) - yorigin
    za(3) = rzu(imol_a,ia_bead3) - zorigin
...
#endif
  end subroutine align_lines_partial_rigid
