...
MODULE moves_simple
  ...
  use energy_pairwise,only:energy,energyjc,calrhoz2
  implicit none
  private
  save
...
!> The attempts are stored in \b bntrax(yz)
  subroutine translation()
    ...
    use sim_cell,only:update_linked_cell
      ...
      real::rx,ry,rz,dchain,ddx,ddy,ddz,vn(nEnergy),vo(nEnergy),deltv,deltvb,rchain,vdum,vrecipo,vrecipn,vTail
...
! calculate the energy of i in the old configuration ***
      ...
      if(ljc(ibox) .and.lz) then
         call energyjc(1,iunit,vTail,imolty,ibox)
         vo(ivTail) = vTail
         vo(ivInterLJ) = vo(ivInterLJ) + vo(ivTail)
         vo(ivTot) = vo(ivTot) + vo(ivTail)
      end if
...
! For ANES algorithm, do the Fluctuating charge moves.
      ...
      if ( deltvb .gt. (2.3E0_dp*softcut) ) then
         if(ljc(ibox) .and.lz) call calrhoz2(imolty,ibox,3)
         return
      end if
...
!> The maximum angular displacement is \b dgamax.
    subroutine rotation()
      ...
      real::rx,ry,rz,dchain,rchain,vn(nEnergy),vo(nEnergy),dgamma,rxorig,ryorig,rzorig,rxnew2,rynew2,rznew2,deltv,deltvb,vdum,vTail
...
! For ANES algorithm, do the Fluctuating charge moves.
      ...
      if ( deltvb .gt. (2.3E0_dp*softcut) ) then 
        !if(ljc(ibox) .and.(lx .or. ly)) call calrhoz2(imolty,ibox,3)
        return
      end if
      if ( deltv .le. 0.0E0_dp ) then
! move accepted
...
!> can specify the specific atoms they want to do translations on.
    ...
    subroutine Atom_translation()
      use sim_particle,only:update_neighbor_list_molecule_v2,ctrmas
      use sim_cell,only:update_linked_cell
...
! update linkcell list
      ...
      if (lneigh.or.lneighbor) then
         call update_neighbor_list_molecule_v2(i) !HF assumes the function  to be called is the same as regular translation, which
                                                  !might be wrong.
      end if
