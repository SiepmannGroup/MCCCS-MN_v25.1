...
!> 2. Joseph Hautman and Michael L Klein, "Simulation of a monolayer of alkyl thiol chains", J Chem Phys 1989,91:4994
  function slitpore(z,ntij)
    ...
    real::z_abs,sig3,dz3,dz12
    
    ! Note that vvdW(1,ntij) is 4*epsilon
    z_abs = abs(z) ![Hsiao-Feng Liu] We should use z_abs instead of z to correctly calculate distance from the surface, |z-0|
    if (surface_type.eq.1) then
       ...
       slitpore = onepi/2.0_dp*rsol*vvdW(1,ntij)*vvdW(3,ntij)*delta*(0.4_dp*(vvdW(2,ntij)/z)**10-(vvdW(2,ntij)/z)**4&
               -vvdW(3,ntij)**2/(3.0_dp*delta*(z+0.61_dp*delta)**3)) ![Hsiao-Feng Liu] I didn't change this since I didn't use it.
