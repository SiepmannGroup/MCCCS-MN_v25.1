...
MODULE sim_particle
  ...
  public::BeadType,AtomType,MoleculeType,check_neighbor_list,init_neighbor_list,add_neighbor_list,add_neighbor_list_molecule&
   ,add_neighbor_list_molecule_v2&
   ,update_neighbor_list_molecule,save_neighbor_list,restore_neighbor_list,allocate_neighbor_list,ctrmas,lnn,lnn_t,neighbor&
   ,neigh_cnt,neigh_o,ndij,nxij,nyij,nzij,neighi,neigh_icnt,ndiji,nxiji,nyiji,nziji&
   ,neighi_v2,neigh_icnt_v2,ndiji_v2,nxiji_v2,nyiji_v2,nziji_v2&
   ,extend_molecule_type,mp_bcast_molecule&
   ,update_coord_in_tree,neighboro,update_neighbor_list_molecule_swatch&
   ,update_neighbor_list_molecule_v2,update_neighbor_list_molecule_v2_swap,update_neighbor_list_molecule_swatch_v2,temp_neighj&
   ,sort_neighbor_list
  ...
  integer,parameter::maxneigh=300 !> CCK: Make sure the maximun cluster size is smaller than this threshold 
  integer::neighi(maxneigh),neigh_icnt,temp_neighj(maxneigh)
  integer,allocatable::neighi_v2(:,:),neigh_icnt_v2(:) !neigh_icnt is reset to 0 in subroutine energy(); neighi(:) no need to reset
  real::ndiji(maxneigh),nxiji(maxneigh),nyiji(maxneigh),nziji(maxneigh)
  real,allocatable::ndiji_v2(:,:),nxiji_v2(:,:),nyiji_v2(:,:),nziji_v2(:,:)
...
contains
  ...
  subroutine add_neighbor_list_molecule_v2(i,j,rij,xij,yij,zij)
    use sim_system,only:io_output
    integer,intent(in)::i,j
    real,intent(in)::rij,xij,yij,zij
    neigh_icnt_v2(i)=neigh_icnt_v2(i)+1
    neighi_v2(neigh_icnt_v2(i),i)=j
    ndiji_v2(neigh_icnt_v2(i),i) = rij
    nxiji_v2(neigh_icnt_v2(i),i) = xij
    nyiji_v2(neigh_icnt_v2(i),i) = yij
    nziji_v2(neigh_icnt_v2(i),i) = zij
  end subroutine add_neighbor_list_molecule_v2

  subroutine update_neighbor_list_molecule(i) ! CK: i is the removed part...
    use sim_system,only:io_output
    integer,intent(in)::i
    integer::ic,j,ip
    logical::lneighij
    ...
    if (lneighbor.or.lgaro) then
       do ic = 1, neigh_cnt(i) ! loop over i's neighbor ! update i's old neighbor first
          j = neighbor(ic,i)
          do ip = 1,neigh_cnt(j) ! loop over j's neighbor
             if (neighbor(ip,j).eq.i) then ! if j's neighbor is i, remove i from j's neighbor list
                neighbor(ip,j)=neighbor(neigh_cnt(j),j) ! updating...but how to update? Use the last index of j's neighbor
       ...
       !After accept, we transfer neigh_icnt and neighi(:) into neigh_cnt(i) and neighbor(:,i), respectively
       !i in new location:
       neigh_cnt(i) = neigh_icnt
       do ic = 1,neigh_icnt ! loop over i's new neighbors ! now updating i itself...
          j = neighi(ic)
          neighbor(ic,i)=j
          ndij(ic,i) = ndiji(ic)
          nxij(ic,i) = nxiji(ic)
          ...
          do ip = 1,neigh_cnt(j)
             if (neighbor(ip,j).eq.i) then
                 lneighij = .true. !If before swap, i is already one of j's neighbor, we don't want to append i to j's list)
             end if
          end do
          if (.not.lneighij) then !Only append i to j's list if i is not one of j's neighbors before swap
             neigh_cnt(j) = neigh_cnt(j)+1
             neighbor(neigh_cnt(j),j) = i
  ...
  subroutine update_neighbor_list_molecule_v2(i)
    !HF: for trans/rot/cbmc moves; use when add_neighbor_list_molecule_v2() is used in energy()
    !Important: when 'energy()' is called by swap(), the id of the involved molecule is nchain+1
    !But for trans/rot/cbmc moves, the id passed to energy() is indeed 'i' itself.
    !Hence, we pass neigh_icnt(i) to neigh_cnt(i) and neighi(:,i) to neighbor(i)
    use sim_system,only:io_output,nchain
    integer,intent(in)::i
    integer::ic,j,ip
    logical::lneighij
    ...
    if (lneighbor.or.lgaro) then
       do ic = 1, neigh_cnt(i) !remove i from i's old neighbors' lists
          j = neighbor(ic,i)
          do ip = 1,neigh_cnt(j) ! loop over j's neighbor
             if (neighbor(ip,j).eq.i) then ! if j's neighbor is i, remove i from j's neighbor list
                neighbor(ip,j)=neighbor(neigh_cnt(j),j) ! updating...but how to update? Use the last index of j's neighbor
       ...
       !After accept, we transfer neigh_icnt_v2(i) and neighi_v2(1:neigh_icnt_v2(i),i) into neigh_cnt(i) and neighbor(1:neigh_cnt(i),i), respectively
       !i in new location:
       neigh_cnt(i) = neigh_icnt_v2(i) !Read the explanation above
       do ic = 1,neigh_cnt(i) ! loop over i's new neighbors ! now updating i itself...
          j = neighi_v2(ic,i)
          neighbor(ic,i)=j
          ndij(ic,i) = ndiji_v2(ic,i)
          nxij(ic,i) = nxiji_v2(ic,i)
          nyij(ic,i) = nyiji_v2(ic,i)
          nzij(ic,i) = nziji_v2(ic,i)
          lneighij = .false.
          do ip = 1,neigh_cnt(j)
             if (neighbor(ip,j).eq.i) then
                 lneighij = .true. !If before swap, i is already one of j's neighbor, we don't want to append i to j's list)
          ...
          if (.not.lneighij) then !Only append i to j's list if i is not one of j's neighbors before swap
             neigh_cnt(j) = neigh_cnt(j)+1
             neighbor(neigh_cnt(j),j) = i
             ndij(neigh_cnt(j),j) = ndiji_v2(ic,i)
             nxij(neigh_cnt(j),j) = -nxiji_v2(ic,i)
             nyij(neigh_cnt(j),j) = -nyiji_v2(ic,i)
             nzij(neigh_cnt(j),j) = -nziji_v2(ic,i)
          end if
       end do
    end if
  end subroutine update_neighbor_list_molecule_v2


  subroutine update_neighbor_list_molecule_v2_swap(i) 
    !HF: for swap move only; use when add_neighbor_list_molecule_v2() is used in energy()
    !Important: when 'energy()' is called by swap(), the id of the involved molecule is nchain+1
    !Hence, we should pass neighi(nchain+1) to neighor(i)
    !In previous version, we don't need to notice this nuance because neighi(:) is just updated and its elements are simply what's
    !in it when last called by energy().
    use sim_system,only:io_output,nchain
    integer,intent(in)::i
    integer::ic,j,ip
    logical::lneighij
    ...
    if (lneighbor.or.lgaro) then
       ...
       !After accept, we transfer neigh_icnt_v2(nchain+1) and neighi_v2(1:neigh_icnt_v2(nchain+1),nchain+1) into neigh_cnt(i) and neighbor(1:neigh_cnt(i),i), respectively
       !i in new location:
       neigh_cnt(i) = neigh_icnt_v2(nchain+1) !Read the explanation above
       do ic = 1,neigh_cnt(i) ! loop over i's new neighbors ! now updating i itself...
          j = neighi_v2(ic,nchain+1)
          neighbor(ic,i)=j
          ndij(ic,i) = ndiji_v2(ic,nchain+1)
          nxij(ic,i) = nxiji_v2(ic,nchain+1)
          nyij(ic,i) = nyiji_v2(ic,nchain+1)
          nzij(ic,i) = nziji_v2(ic,nchain+1)
          lneighij = .false.
          do ip = 1,neigh_cnt(j)
             if (neighbor(ip,j).eq.i) then
                 lneighij = .true. !If before swap, i is already one of j's neighbor, we don't want to append i to j's list)
          ...
          if (.not.lneighij) then !Only append i to j's list if i is not one of j's neighbors before swap
             neigh_cnt(j) = neigh_cnt(j)+1
             neighbor(neigh_cnt(j),j) = i
             ndij(neigh_cnt(j),j) = ndiji_v2(ic,nchain+1)
             nxij(neigh_cnt(j),j) = -nxiji_v2(ic,nchain+1)
             nyij(neigh_cnt(j),j) = -nyiji_v2(ic,nchain+1)
             nzij(neigh_cnt(j),j) = -nziji_v2(ic,nchain+1)
          end if
       end do
    end if
  end subroutine update_neighbor_list_molecule_v2_swap


  subroutine update_neighbor_list_molecule_swatch(i,j) 
    use sim_system,only:nchain,io_output
    integer,intent(in)::i,j
    integer::ic,k,ip,temp
    logical::lneighij
    ...
    if (lneighbor.or.lgaro) then
       !write(io_output,*) 'CK is here, updating swatch neighbor...'
       do ic = 1, neigh_cnt(i)
          k = neighbor(ic,i) !i's neighbors
          do ip = 1,neigh_cnt(k)
             if (neighbor(ip,k).eq.i) then
                neighbor(ip,k)=nchain+1 !a temporary number, which will be replaced by "j"
                exit
             end if
          end do
       end do
       do ic = 1, neigh_cnt(j)
          k = neighbor(ic,j) !j's neighbor
          do ip = 1,neigh_cnt(k)
             if (neighbor(ip,k).eq.j) then
                neighbor(ip,k)=i
                exit
             end if
          end do
      ...
      do ic = 1, neigh_cnt(i)
          k = neighbor(ic,i) !back to i's neighbors
          do ip = 1,neigh_cnt(k)
             if (neighbor(ip,k).eq.(nchain+1)) then !now replace nchain+1 with j
                neighbor(ip,k)=j
                exit
             end if
          end do
      end do
      !Reminder: neigh_cnt(i/j) should not change since i<-> j, so we don't need to change them.

      do ic = 1, neigh_cnt(i) !store old neighborlist of i to neighboro(:,i)
                neighboro(ic,i)=neighbor(ic,i)
                ndijo(ic,i) = ndij(ic,i)
                nxijo(ic,i) = nxij(ic,i)
                nyijo(ic,i) = nyij(ic,i)
                nzijo(ic,i) = nzij(ic,i)
      end do
      temp=neigh_cnt(i)
      !neigh_cnt(i)=neigh_cnt(j)
      do ip = 1,neigh_cnt(j)
                neighbor(ip,i)=neighbor(ip,j)
                ndij(ip,i) = ndij(ip,j)
                nxij(ip,i) = nxij(ip,j)
                nyij(ip,i) = nyij(ip,j)
                nzij(ip,i) = nzij(ip,j)
      ...
      !neigh_cnt(i)=neigh_cnt(j)
      do ip = 1,neigh_cnt(i)
                neighbor(ip,j)=neighboro(ip,i)
                ndij(ip,j) = ndijo(ip,j)
                nxij(ip,j) = nxijo(ip,j)
                nyij(ip,j) = nyijo(ip,j)
                nzij(ip,j) = nzijo(ip,j)
  ...
  end subroutine update_neighbor_list_molecule_swatch

  subroutine update_neighbor_list_molecule_swatch_v2(i,j) !HF
    use sim_system,only:io_output,nboxi
    integer,intent(in)::i,j
    integer::ic,ip,k
    logical::lneighij
    ...
    if (lneighbor.or.lgaro) then
       do ic = 1, neigh_cnt(i) ! loop over i's old neighbors
          k = neighbor(ic,i)
          !if (ic==1) then
          !        write(io_output,*) "old",i," 1st neighbor(",k,") before update:", neighbor(1:neigh_cnt(k),k)
          !        do ip = 1, neigh_cnt(k)
          !            write(io_output,*) 'nboxi:',nboxi(neighbor(ip,k))
          !        end do
          !end if
          do ip = 1,neigh_cnt(k) ! loop over k's neighbor
             if (neighbor(ip,k).eq.i) then ! if k's neighbor is i, remove i from k's neighbor list
                neighbor(ip,k)=neighbor(neigh_cnt(k),k) ! but how to remove? Use the last index of k's neighbor
                ndij(ip,k) = ndij(neigh_cnt(k),k)
                nxij(ip,k) = nxij(neigh_cnt(k),k)
                nyij(ip,k) = nyij(neigh_cnt(k),k)
                nzij(ip,k) = nzij(neigh_cnt(k),k)
                neigh_cnt(k) = neigh_cnt(k)-1
                exit
             end if
          end do
          !if (ic==1) then
          !        write(io_output,*) "old",i," 1st neighbor(",k,") after update:", neighbor(1:neigh_cnt(k),k)
          !end if
       end do
       do ic = 1, neigh_cnt(j) !loop over j's old neighbors
          k = neighbor(ic,j)
          do ip = 1,neigh_cnt(k) ! loop over k's neighbor
             if (neighbor(ip,k).eq.j) then ! if k's neighbor is j, remove j from k's neighbor list
                neighbor(ip,k)=neighbor(neigh_cnt(k),k) ! but how to remove? Use the last index of k's neighbor
                ndij(ip,k) = ndij(neigh_cnt(k),k)
                nxij(ip,k) = nxij(neigh_cnt(k),k)
                nyij(ip,k) = nyij(neigh_cnt(k),k)
       ...
       !Next, we transfer neigh_icnt(i/j) and neighi(:,i/j) to neigh_cnt(j/i) and neighbor(:,j/i), respectively
       !Interestingly, when interbox swatch happens, energy() subroutine actually swap the numeric label of i and j. Hence, the
       !information of i is actually store as j, and vice versa.
       !i in new location:
       !neigh_cnt(i) = neigh_icnt_v2(i)
       neigh_cnt(i) = neigh_icnt_v2(j)
       !write(io_output,*) 'last neighi(',i,')=',neighi_v2(1:neigh_icnt_v2(j),j)
       do ic = 1,neigh_icnt_v2(j) ! loop over i's new neighbors
          k = neighi_v2(ic,j)
          neighbor(ic,i)=k
          ndij(ic,i) = ndiji_v2(ic,j)
          nxij(ic,i) = nxiji_v2(ic,j)
          nyij(ic,i) = nyiji_v2(ic,j)
          nzij(ic,i) = nziji_v2(ic,j)
          lneighij = .false.
          do ip = 1,neigh_cnt(k)
             if (neighbor(ip,k).eq.i) then
                 lneighij = .true. !If before swap, i is already one of k's neighbor, we don't want to append i to k's list
             end if
          end do
          if (.not.lneighij) then !Only append i to k's list if i is not one of k's neighbors before swap
             neigh_cnt(k) = neigh_cnt(k)+1
             neighbor(neigh_cnt(k),k) = i
             ndij(neigh_cnt(k),k) = ndiji_v2(ic,i)
             nxij(neigh_cnt(k),k) = -nxiji_v2(ic,i)
             nyij(neigh_cnt(k),k) = -nyiji_v2(ic,i)
             nzij(neigh_cnt(k),k) = -nziji_v2(ic,i)
          end if
       end do
       !Now, for j
       neigh_cnt(j) = neigh_icnt_v2(i)
       do ic = 1,neigh_icnt_v2(i) ! loop over j's new neighbors
          k = neighi_v2(ic,i)
          neighbor(ic,j)=k
          ndij(ic,j) = ndiji_v2(ic,i)
          nxij(ic,j) = nxiji_v2(ic,i)
          nyij(ic,j) = nyiji_v2(ic,i)
          nzij(ic,j) = nziji_v2(ic,i)
          lneighij = .false.
          do ip = 1,neigh_cnt(k)
             if (neighbor(ip,k).eq.j) then
                 lneighij = .true. !If before swap, j is already one of k's neighbor, we don't want to append j to k's list
             end if
          end do
          if (.not.lneighij) then !Only append j to k's list if j is not one of k's neighbors before swap
             neigh_cnt(k) = neigh_cnt(k)+1
             neighbor(neigh_cnt(k),k) = j
             ndij(neigh_cnt(k),k) = ndiji_v2(ic,j)
             nxij(neigh_cnt(k),k) = -nxiji_v2(ic,j)
             nyij(neigh_cnt(k),k) = -nyiji_v2(ic,j)
             nzij(neigh_cnt(k),k) = -nziji_v2(ic,j)
          end if
       end do
    end if

  end subroutine update_neighbor_list_molecule_swatch_v2
  ...
  subroutine sort_neighbor_list(imol)
        ! Subroutine to sort the first neigh_cnt(imol) elements of neighbor(o) in ascending order
        integer, intent(in) :: imol             
        integer :: i, j
        real :: temp

        ! Bubble Sort for neighbor(imol)
        do i = 1, neigh_cnt(imol) - 1
            do j = 1, neigh_cnt(imol) - i
                if (neighbor(j,imol) > neighbor(j+1,imol)) then
                    ! Swap elements
                    temp = neighbor(j,imol)
                    neighbor(j,imol) = neighbor(j+1,imol)
                    neighbor(j+1,imol) = temp
                end if
            end do
        end do
        ! Bubble Sort for neighboro(imol)
        do i = 1, neigh_cnt(imol) - 1
            do j = 1, neigh_cnt(imol) - i
                if (neighboro(j,imol) > neighboro(j+1,imol)) then
                    ! Swap elements
                    temp = neighboro(j,imol)
                    neighboro(j,imol) = neighboro(j+1,imol)
                    neighboro(j+1,imol) = temp
                end if
            end do
        end do
  ...
  subroutine allocate_neighbor_list()
    ...
    if (allocated(lnn)) deallocate(lnn,lnn_t,upnn,upnnsq,upnndg,neighbor,neigh_cnt,neighboro,neigh_o,ndij,nxij,nyij,nzij&
            ,ndijo,nxijo,nyijo,nzijo,neighi_v2,neigh_icnt_v2,ndiji_v2,nxiji_v2,nyiji_v2,nziji_v2,stat=jerr)
    allocate(lnn(nmax,nmax),lnn_t(nmax,nmax),upnn(nbxmax),upnnsq(nbxmax),upnndg(ntmax,nbxmax),neighbor(maxneigh,nmax)&
     ...
     ,nzij(maxneigh,nmax),ndijo(maxneigh,nmax),nxijo(maxneigh,nmax),nyijo(maxneigh,nmax),nzijo(maxneigh,nmax)&
     ,neighi_v2(maxneigh,nmax),neigh_icnt_v2(nmax),ndiji_v2(maxneigh,nmax),nxiji_v2(maxneigh,nmax),nyiji_v2(maxneigh,nmax)&
     ,nziji_v2(maxneigh,nmax),stat=jerr)
