...
!==========================================================!
  ...
  integer,parameter::nvib_max=20,nben_max=20,ntor_max=20,max_num_rigid_beads=24
  ...
  logical,allocatable::ljc(:)
  real,allocatable::rhoz(:,:,:,:),rhozo(:,:,:,:),rhozfi(:,:,:,:)      
  ...
  real,allocatable::pmrotbd(:,:),rxp(:,:),ryp(:,:),rzp(:,:)& !< (iunit,itrial) coordinates of iunit of the selected molecule for the itrial-th candidate configuration, for use in rosenbluth
   ...
   ,xvec(:,:),yvec(:,:),zvec(:,:)&
   ,rxflex(:),ryflex(:),rzflex(:) !< coordinates of trial flexible chain determined by regular CBMC (1 to igrow) , added by CK Chang
...
CONTAINS
  subroutine allocate_system()
    integer,parameter::initial_size=30
    integer::jerr

    if (allocated(boxlx)) deallocate(boxlx,boxly,boxlz,rcut,rcutnn,kalp,lsolid,lrect,ljc,express,ghost_particles&
    ...
    allocate(nrotbd(ntmax),xcm(nmax),ycm(nmax),zcm(nmax),pmsatc(npamax),pmswtcb(npamax,npabmax),nswatb(npamax,2)&
     ...
     ,rmexpc(ntmax),eetype(ntmax),ncmt(nbxmax,ntmax),ncmt2(nbxmax,ntmax,40),parall(ntmax,nmax)&
  ...
  subroutine allocate_rhoz()
    integer::jerr
    if (allocated(boxlx)) deallocate(rhoz,rhozfi,rhozo,stat=jerr)
    allocate(rhoz(jcbin,nmolty,40,nbxmax),rhozo(jcbin,nmolty,40,nbxmax),rhozfi(jcbin,nmolty,40,nbxmax))
  ...
  subroutine allocate_molecule()
    ...
    if (allocated(splist)) deallocate(splist,lexist,lexclu,a15type,epsilon_f,sigma_f,ljscale,qscale2,ee_qqu&
     ,rxnew,rynew,rznew,rxflex,ryflex,rzflex,rxu,ryu,rzu,qqu,rxuion,ryuion,rzuion,qquion,rxu_update,ryu_update,rzu_update,linclu&
    ...
    allocate(splist(npamax,numax,2),lexist(numax),lexclu(ntmax,numax,ntmax,numax),a15type(ntmax,numax,numax)&
     ...
     ,rxflex(numax),ryflex(numax),rzflex(numax)&
