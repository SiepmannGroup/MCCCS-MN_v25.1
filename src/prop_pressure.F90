...
!> energy), corp (tail corrections to pressure)
  subroutine pressure(press,surf,comp,ibox)
    ...
    integer::i,imolty,j,jmolty,ii,jj,ntii,ntjj,ntij,iii,jjj,k,bindex
    logical::lqimol,lexplt,lij2,lqjmol,lcoulo(numax,numax)
    real::corpx,corpz,corpxx,corpzz
...
! ################################################################
    ...
    if(ljc(ibox)) then
        ...
        corpzz = 0.0_dp
         do bindex = 1, jcbin
            do imolty=1,nmolty
               do ii = 1,nunit(imolty)
                  call jccorp(bindex,imolty,ii,ibox,corpx,corpz)
                  corpxx = corpxx + corpx
                  corpzz = corpzz + corpz
             end do
          end do
        end do
         press = press - 2.0_dp*corpxx - corpzz
         pxx = pxx + corpxx
         pyy = pyy + corpxx
         pzz = pzz + corpzz
    end if
    ...
    if ((ltailc.and..not.ljc(ibox)).or.(.not.lshift.and.numberDimensionIsIsotropic(ibox).eq.3)) then
...
!> energy), prop_pressure::pressure (force calculation)
    ...
    do ii = 1, nunit(imolty)
       ...
       do jj = 1, nunit(jmolty)
          ...
          if (nonbond_type(ntij).eq.1) then
             ...
             rci3 = tmp/rbcut3
             rci6 = rci3*rci3
             if (ltailc.and..not.ljc(ibox)) then ! tail corrections
             ...
             ! MMFF94 or DPD
             if (ltailc.and..not.ljc(ibox)) then ! tail corrections for MMFF94 are not implemented yet
             else if (.not.lshift.and.numberDimensionIsIsotropic(ibox).eq.3) then ! impulsive force corrections
                if (vvdW(2,ntij).ne.0) then
 ...
 !calculate the long range correction associated with one bead
  subroutine jccorp(bindex,imolty,ii,ibox,corpx,corpz)
  real::corpx,corpz
  integer,intent(in)::bindex,imolty,ii,ibox
  integer::k,jj,jmolty,ntii,ntjj,ntij
  real::factor,rbcut,rbcutsq,width,halflz,volbin,rhozk,sigma,sigma2,epsilon2,drzu,abdrzu,abdrzusq,wikxx,wikzz,tmp,tmp2,tmp6,tmp12
  ...
  ntii = ntype(imolty,ii)
   ...
   do k=1,jcbin
      do jmolty=1,nmolty
         do jj=1,nunit(jmolty)
            rhozk = rhoz(k,jmolty,jj,ibox)
            ntjj=ntype(jmolty,jj)
            ntij=type_2body(ntii,ntjj)
            sigma=vvdW(2,ntij)
            sigma2=vvdW(3,ntij)
            epsilon2=vvdW(1,ntij)
            drzu = dble((bindex-k))*width
            abdrzu = abs(drzu)
            if (abdrzu .gt. halflz) abdrzu = boxlz(ibox)-abdrzu
            abdrzusq = abdrzu * abdrzu
            if(abdrzu .le. rbcut) then
               tmp = sigma/rbcut
               tmp2 = tmp*tmp
               tmp6 = tmp2*tmp2*tmp2
               tmp12 = tmp6 * tmp6
               wikxx = onepi*epsilon2*((0.6_dp*rbcutsq - 0.5_dp*abdrzusq)*tmp12-(0.75_dp*rbcutsq-0.5_dp*abdrzusq)*tmp6)
               wikzz = onepi*epsilon2*abdrzusq*(tmp12-tmp6)
               corpx = corpx + wikxx * rhozk * width
               corpz = corpz + wikzz * rhozk * width
            else
               tmp = sigma/abdrzu
               tmp2 = tmp*tmp
               tmp6 = tmp2*tmp2*tmp2
               tmp12 = tmp6 * tmp6
               wikxx = onepi*epsilon2*abdrzusq*(0.1_dp*tmp12-0.25_dp*tmp6)
               wikzz = onepi*epsilon2*abdrzusq*(tmp12-tmp6)
               corpx = corpx + wikxx * rhozk * width
               corpz = corpz + wikzz * rhozk * width
             end if
