...
module transfer_swap
  ...
  use energy_pairwise,only:energy,boltz,coru,energyjc2,jccoru,calrhoz4,energyjc,calrhoz2
...
!> \author Rewritten from old swap and swapbr subroutines by M.G. Martin (9-18-97)
  subroutine swap()
    use sim_particle,only:update_neighbor_list_molecule_v2,ctrmas,neigh_cnt,neighbor,neigh_icnt,update_coord_in_tree,temp_neighj&
            ,neigh_icnt_v2,update_neighbor_list_molecule_v2_swap
    ...
    real::v1ins(nEnergy),v1rem(nEnergy),w1ins,w1rem,wnlog,wolog,wdlog,wratio,vinsta,vremta,vintrata,volins,volrem,rho,arg
    real::rvol,x,y,z,rijsq,wbias_ins,wbias_rem,r,xi1,xi2,xisq
    real::vrecipn,vrecipo,vdum,whins,whrem,vTailn,vTailo
...
197       pointp = int(real(ncmt(boxrem,imolty),dp)*random(-1)) + 1
       ...
       !write(io_output,*) 'pointp=',pointp, 'particle ',irem,' is being removed, imolty is:',imolty,' and the box is:',boxrem,&
       !'boxins=', boxins !HF

! ===>  for both gibbs and grand-canonical we have:
! insert a chain in box: boxins
! remove one in box: boxrem
 !write(io_output,*) 'boxrem',boxrem,' imolty',imolty,' lempty',lempty !HF
...
! First AVBMC Steps *
! *********************
       rmol = random(-1)  ! draw moltyp for j
       ldone = .false.
       !jmolty = 1 !CK add
       do imol = 1, nmolty
       ...
       !write(io_output,*) 'CK is here. boxins jmolty ncmt ',boxins, jmolty, ncmt(boxins,jmolty) !HF
...
111       pointp2=int( dble(ncmt(boxins,jmolty))*random(-1))+1     ! draw target j
...
! end do
          ...
          !neighk_num = neigh_cnt(kins) !original
          neighk_num = 0
          do imol = 1, neigh_cnt(kins) ! add a loop to sum up all particle can perform AVBMC moves in the 'in' region
                if (moltyp(neighbor(imol,kins)) .eq. imolty )then
                neighk_num = neighk_num + 1
                end if
          end do
       end if
       if ( lavbmc2(imolty) .or. lavbmc3(imolty) ) then
          !neighj_num = neigh_cnt(jins) !original
          ...
          if (lavbmc3(imolty) .and. kmolty .eq. imolty) then
             ...
             neighj_num = 0
             do imol = 1, neigh_cnt(jins) ! add a loop to sum up all species i around the target j.
                !write(io_output,*) 'jins= ',jins,' imol= ', imol,'  particle id= ',neighbor(imol,jins),' nei typ&
                !',moltyp(neighbor(imol,jins))!HF
                temp_neighj(imol) = 0 
                if (moltyp(neighbor(imol,jins)).eq.imolty)then
                neighj_num = neighj_num + 1 ! count number of i around j
                temp_neighj(neighj_num) = neighbor(imol,jins) ! save the particle id of species i around j
                end if
             end do
       end if
...
! out -> j case
             wbias_rem=wbias_rem /dble(ncmt(boxrem,imolty)-neighj_num-joffset) ! how many molecules are there is the out region of imolty?
             lrem_out = .true.
119          pointp=int(dble(ncmt(boxrem,imolty))*random(-1))+1 ! draw particle i in the out region
             irem = parbox(pointp,boxrem,imolty)
             if ( irem .eq. jins ) goto 119
             !write(io_output,*) 'pointp=',pointp,'irem=',irem,'ncmt(boxrem,imolty)=',ncmt(boxrem,imolty),&
             !                   'neighj_num=',neighj_num,'joffset=',joffset !HF
             x = rxu(irem,1) - rxu(jins,1)
             y = ryu(irem,1) - ryu(jins,1)
             z = rzu(irem,1) - rzu(jins,1)
             if ( lpbc ) call mimage(x,y,z,boxins)
             rijsq = x*x + y*y + z*z
             if (rijsq.lt.rbsmax**2.and.rijsq.gt.rbsmin**2) then ! CK plz check here...
                 !if (ncmt(boxrem,imolty).ne.1) then ! original, weird condition...
                 if (ncmt(boxrem,imolty).ne.(neighj_num+joffset)) then 
                     goto 119
                 else
                     lempty = .true.
                     return
...
113             pointp=int(dble(neigh_cnt(kins))*random(-1))+1      ! CK please check here
! irem = neighbor(pointp,kins,imolty)
! write(io_output,*) 'kins,irem:',kins,irem,neighk_num
                irem = neighbor(pointp,kins)
                if ( moltyp(irem) .ne. imolty ) goto 113 ! CK modified here ensure the picked particle has correct moltyp
...
! write(io_output,*) 'jins,irem:',jins,irem,neighj_num
                ...
                irem = temp_neighj(pointp) ! CK change here
...
!> \bug: previously there was no check
                ...
                !neighj_num is a small number. Redraw too early in the in region shifts eqlm to clustering...
                !   goto 114 
                ...
                if ( ibox .ne. boxrem ) then
                   write(io_output,*) 'problem in swap'
                   write(io_output,*) 'irem= ',irem,' ibox= ',ibox,' boxrem=',boxrem ! CK added here for debugging
                   write(io_output,*) 'pointp=',pointp,' jins= ',jins, 'nboxi(jins) ', nboxi(jins)  ! CK added here for debugging
                   write(io_output,*) 'neighj_num=',neighj_num ! CK added here for debugging
                   call err_exit(__FILE__,__LINE__,'',myid+1)
                end if
                iins = irem
             end if
...
190    waddnew = waddnew * whins
    ...
    if (ltailc.and.lswapinter.and..not.lideal(boxins).and..not.ljc(boxins)) then
    ! END JLR 11-24-09
       vinsta = 0.0E0_dp
       do jmt = 1, nmolty
       ...
       vinsta = 0.0E0_dp !CK turn off for avbmc
    end if
    ! End Tail corrections for BOXINS with inserted particle
    if(ljc(boxins).and.lswapinter.and..not.lideal(boxins).and.ltailc) then
       call energyjc2(.TRUE.,1,igrow,vTailo,imolty,boxins)
       call calrhoz4(.TRUE.,imolty,boxins,1)
       call calrhoz4(.TRUE.,imolty,boxins,2)
       call energyjc2(.TRUE.,1,igrow,vTailn,imolty,boxins)
       vinsta = vTailn - vTailo
       waddnew = waddnew*exp(-beta*vinsta)
       vnew(ivTot) = vnew(ivTot) + vinsta
       vnew(ivInterLJ) = vnew(ivInterLJ) + vinsta
    end if
...
#endif
       if(ljc(boxins).and.lswapinter) call calrhoz4(.TRUE.,imolty,boxins,3) !CK avbmc
       !if(ljc(boxins).and..not.lswapinter) call calrhoz2(imolty,boxins,3) !CK avbmc (may be turn this off)...
       return
    else
...
! determine whether it is in or out
          ...
          if ( lrem_out ) then
! out-> j case
             wbias_ins = (1-pmbias(imolty))/ (boxlx(boxins)*boxly(boxins)*boxlz(boxins)- vol_eff) ! CK check here for AVBMC acc
             if (lavbmc2(imolty) .or. lavbmc3(imolty) )  wbias_ins=wbias_ins/dble(neighj_num+1) ! CK check here for AVBMC acc
...
! selected from the region bounded by jins
                ...
                if ( lavbmc2(imolty) )  wbias_ins=wbias_ins/dble( ncmt(boxrem,imolty)-neighj_num-joffset+1) !CK check here for AVBMC acc 
...
! j -> out case
                      wbias_ins=wbias_ins/dble( ncmt(boxrem,imolty)-neighj_num-joffset+1)  !CK check here for AVBMC acc
                   end if
                end if
             end if
...
#endif
       if(ljc(boxins).and.lswapinter) call calrhoz4(.TRUE.,imolty,boxins,3) !CK avbmc
       !if(ljc(boxins).and..not.lswapinter) call calrhoz2(imolty,boxins,3) !CK avbmc (maybe turn this off)
       return
    end if
...
!> \bug problem here on calculation of favor and favor2
    ...
    if (ltailc.and.lswapinter.and..not.lideal(boxrem).and..not.ljc(boxrem)) then
    ! END JLR 11-24-09
       ...
       vremta = 0.0E0_dp !CK truned off for avbmc
    end if
    ! End of intermolecular tail correction for boxrem
    if(ljc(boxrem).and.lswapinter.and..not.lideal(boxrem).and.ltailc)  then
       call energyjc2(.FALSE.,1,igrow,vTailo,imolty,boxrem)
       call calrhoz4(.FALSE.,imolty,boxrem,1)
       call calrhoz4(.FALSE.,imolty,boxrem,2)
       call energyjc2(.FALSE.,1,igrow,vTailn,imolty,boxrem)
       vremta = vTailo - vTailn
       waddold = waddold*exp(-beta*vremta)
       vold(ivTot) = vold(ivTot) + vremta
       vold(ivInterLJ) = vold(ivInterLJ) + vremta
    end if
    if(ljc(boxrem).and..not.lswapinter.and..not.lideal(boxrem).and.ltailc)  then ! CK try for avbmc
       call energyjc(1,igrow,vTailo,imolty,boxrem)
       call calrhoz2(imolty,boxrem,2)
       call calrhoz2(imolty,boxrem,1)
       call energyjc(1,igrow,vTailn,imolty,boxrem)
       vremta = vTailo
       vinsta = vTailn
       waddold = waddold*exp(-beta*vremta)
       vold(ivTot) = vold(ivTot) + vremta
...
1000 if ( wdlog .lt. -softcut ) then
       ...
       if (ljc(boxins).and.lswapinter) call calrhoz4(.TRUE.,imolty,boxins,3) !CK avbmc
       if (ljc(boxrem).and.lswapinter)  call calrhoz4(.FALSE.,imolty,boxrem,3)
       if (ljc(boxins).and..not.lswapinter) call calrhoz2(imolty,boxins,3) !CK avbmc
       if (ljc(boxrem).and..not.lswapinter)  call calrhoz2(imolty,boxrem,3)
    ...
    if (.not. lswapinter) then
       ...
       if ( neigh_old .le. 60 .and. neigh_icnt_v2(nchain+1) .le. 60 ) then !CK try 6 to 60 !HF: neigh_icnt->neigh_icnt_v2(nchain+1)
          if ( lavbmc1(imolty) ) then
             if ( lrem_out ) then
                if ( lins_in ) then
                   ip = 1
 ...
 !write(io_output,*) neigh_old,neigh_icnt,ip !CK debug start
 ...
 !end if !CK debug end
          ...
          !if (neigh_old .eq. 0 .and. neigh_icnt .eq. 1) then   !CK bypass cnt_wra1 cnt_wra2 starts
          !   wdlog = log10 (wratio)
          ...
          !end if  ! CK bypass cnt_wra1 cnt_wra2 endl
       end if
    end if
...
! update dipole term
       ...
       if (lneigh.or.lneighbor.or.lgaro) then
          if (boxrem .eq. boxins) then !if this is an avbmc2/3 move
              call update_neighbor_list_molecule_v2(irem) !energy(i) is indeed i, not nchain+1
          else
              call update_neighbor_list_molecule_v2_swap(irem)!energy(i) is not i but nchain+1
          end if
          !write(io_output,*) 'update neighbor(', irem,')=',neighbor(1:neigh_cnt(irem)+2,irem)
       end if
    else  
       if (lgrand.and.boxins.eq.1)  call gcmc_cleanup(imolty,boxrem)
       !if (ljc(boxins))   call calrhoz4(.TRUE.,imolty,boxins,3)  !restore
       !if (ljc(boxrem))   call calrhoz4(.FALSE.,imolty,boxrem,3)
        if (ljc(boxins).and.lswapinter)   call calrhoz4(.TRUE.,imolty,boxins,3) 
        if (ljc(boxrem).and.lswapinter)   call calrhoz4(.FALSE.,imolty,boxrem,3)
        if (ljc(boxins).and..not.lswapinter)   call calrhoz2(imolty,boxins,3)
        if (ljc(boxrem).and..not.lswapinter)   call calrhoz2(imolty,boxrem,3)
  ...
  subroutine cnt()  
    ...
    if (lpr.and.myid.eq.rootid) then
       do nnn = 1,4
          ...
          !do i = 1,1000 !CK bypases cnt_wra1 cnt_wra2
