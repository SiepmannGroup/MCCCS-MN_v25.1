...
MODULE energy_pairwise
  ...
  private
  save
  public::sumup,energy,boltz,coru,read_ff,init_ff,U2,type_2body,vdW_nParameter,nonbond_type,energy_inter_com_kd_tree,jcsum,energyjc,energyjc2,jccoru,calrhoz,calrhoz2,calrhoz3,calrhoz4
...
1024                   continue
                       ...
                       do ii = 1,nunit(imolty)
                          ...
                          bead2: do jj = 1, nunit(jmolty)
                             ...
                             rijsq=(rxuij*rxuij)+(ryuij*ryuij)+(rzuij*rzuij)
                             !if (rijsq.lt.rminsq .and..not.((abs(vvdW_b(1,ntii)).lt.1e-6.and.abs(qelect(ntii)).lt.1e-6).or.&
                                !(abs(vvdW_b(1,ntjj)).lt.1e-6.and.abs(qelect(ntjj)).lt.1e-6)).and..not.(lexpand(imolty).or.lexpand(jmolty))) then !CK modified here
...
! KEA garofalini 3 body potential
       ...
       if (ltailc.and..not.ljc(ibox)) then
        ...
        if(ljc(ibox)) then ! use JC correction
          call jcsum(v(ivTail),ibox)
          v(ivInterLJ) = v(ivInterLJ) + v(ivTail)
        end if
    end if
!$$$c      write(io_output,*)
...
! KNS 11-23-15 insert rxui, ryui rzui from above
                      ...
                      if (linclu(imolty,ii,jj)) then
                         !if (rijsq.lt.rminsq.and..not.((abs(vvdW_b(1,ntii)).lt.1e-6.and.abs(qelect(ntii)).lt.1e-6).or.&
                         !       (abs(vvdW_b(1,ntjj)).lt.1e-6.and.abs(qelect(ntjj)).lt.1e-6)).and..not.lexpand(imolty)) then !CK modified here
...
! write(io_output,*) 'v in sumup',v(ivTot)
    ...
    if ( .not. lvol.and.myid.eq.rootid ) then
       ...
       if (ltailc .or. ljc(ibox)) write(io_output,*) 'Tail correction ', v(ivTail)
...
!*****************************************************************
  subroutine energy(i,imolty,v,flagon,ibox,istart,iuend,lljii,ovrlap,ltors,lcharge_table,lfavor,lAtom_traxyz)
    use sim_particle,only:lnn,lnn_t,neighbor,neigh_cnt,ndij,nxij,nyij,nzij,ctrmas,add_neighbor_list_molecule,neighi,neigh_icnt&
     ...
     ,add_neighbor_list_molecule_v2,neigh_icnt_v2!HF
...
! --------------------------------------------------------------------
    ...
    if (flagon.eq.2) then
       ...
       if (lneighbor.or.lgaro) then !HF
           neigh_icnt_v2(i) = 0 
           neigh_icnt_v2(nchain+1) = 0
       end if
    end if
...
2047            continue
                ...
                do ii = istart, iuend
                    ...
                    do jj = 1, nunit(jmolty)
                        ...
                        rijsq = rxuij*rxuij + ryuij*ryuij + rzuij*rzuij
                        !if (rijsq.lt.rminsq.and..not.((abs(vvdW_b(1,ntii)).lt.1e-6.and.abs(qelect(ntii)).lt.1e-6).or.&
                        !        (abs(vvdW_b(1,ntjj)).lt.1e-6.and.abs(qelect(ntjj)).lt.1e-6)).and. .not.(lexpand(imolty).or.lexpand(jmolty))) then ! CK exclude dummy sites
                        if (rijsq.lt.rminsq .and..not.(lexpand(imolty).or.lexpand(jmolty))) then
                            ovrlap = .true.
                        ! RP added for MPI
                        ! return
                        ...
                        if (lneighbor.and.ii.eq.1.and.jj.eq.1.and.flagon.eq.2.and.rijsq.lt.rbsmax**2.and.rijsq.gt.rbsmin**2) then
                            !call add_neighbor_list_molecule(j,sqrt(rijsq),rxuij,ryuij,rzuij) !original
                            !write(io_output,*) 'i=',i !HF: for interbox swap: always nchain+1 and therefore, neighi(:,i) is kept as
                                                      !i=nchain+1, not the actual i involved in swap().
                                                      !For intrabox swap (avbmc2/3) and all other moves, i is just i
                            !write(io_output,*) 'i=',i,',ibox=',ibox,',imolty=',imolty,'rxui=',rxui,',ryui=',ryui,',rzui=',rzui
                            call add_neighbor_list_molecule_v2(i,j,sqrt(rijsq),rxuij,ryuij,rzuij) !HF
...
!KNS 11-23-15 insert rxui, ryui, rzui determined above
          ...
          if ( linclu(imolty,ii,jj) ) then
             if (lljii) then
                !if (rijsq.lt.rminsq .and..not.((abs(vvdW_b(1,ntii)).lt.1e-6.and.abs(qelect(ntii)).lt.1e-6).or.&
                !                (abs(vvdW_b(1,ntjj)).lt.1e-6.and.abs(qelect(ntjj)).lt.1e-6)) .and. .not.lexpand(imolty)) then ! CK exclude dummy sites
                if (rijsq.lt.rminsq .and..not.(lexpand(imolty))) then
                   ovrlap = .true.
    ...
    do itrial = my_start,my_end
       ...
       if(L_Coul_CBMC.and.lewald.and..not.lideal(ibox)) then
          do count = 1,ntogrow
             ...
             !write(io_output,*) 'CK is here to check boltz Ewald count',count,'imolty',imolty,'ii',ii,'v(ivEwald)',v(ivEwald)
          end do
       end if
...
96                 if (L_Coul_CBMC.and.lqinclu(imolty,ii,iu).and.lqchg(ntii).and.lqchg(ntjj).and.rijsq.lt.rcinsq) then
                ...
                else if (L_Coul_CBMC.and.lewald.and..not.lideal(ibox)) then
                   ! will only add correction here if lqinclu is false.
                   ! ewald sum correction term
                   ! CK added check potint
                   if (rijsq.gt.1e-12) then ! CK added
                        rij = sqrt(rijsq)
                        corr = qqu(icharge,ii)*qqu(icharge,iu)*(erfunc(calp(ibox)*rij)-1.0E0_dp) /rij
                        v(ivEwald) = v(ivEwald) + corr
                   else 
                        !write(io_output,*) 'CK is here catching ovrlap icharge',icharge,'ii',ii,'iu',iu,'ibox',ibox
                   end if ! CK added 
                end if
             end do
          end do
          ...
          if (L_Coul_CBMC.and.lewald.and.ntogrow.gt.1.and..not.lideal(ibox)) then
             ...
             do cnt = 1,ntogrow-1
                ...
                do count = cnt+1,ntogrow
                   ...
                   corr = qqu(icharge,ii)*qqu(icharge,iu)*(erfunc(calp(ibox)*rij)-1.0E0_dp)/rij
                   v(ivEwald) = v(ivEwald) + corr
                   !write(io_output,*) 'CK is here checking v(ivEwald)',v(ivEwald),'rij',rij,'calp',calp(ibox),'qqu ii',qqu(icharge,ii)&
                   !     ,'qqu iu',qqu(icharge,iu),'corr',corr
                end do
             end do
          end if
       end if
...
108                 do count = 1,ntogrow
                       ...
                       do jj = 1,nunit(jmolty)
                          ...
                          !if (rijsq.lt.rminsq.and..not.((abs(vvdW_b(1,ntii)).lt.1e-6.and.abs(qelect(ntii)).lt.1e-6).or.&
                          !      (abs(vvdW_b(1,ntjj)).lt.1e-6.and.abs(qelect(ntjj)).lt.1e-6)).and..not.(lexpand(imolty).or.lexpand(jmolty))) then !CK excludes dummy sites
                          if (rijsq.lt.rminsq .and..not.(lexpand(imolty).or.lexpand(jmolty))) then
                             my_lovr(my_itrial) = .true.
                             ...
                             !if(rijsq.gt.rminsq) then !CK added
                                v(ivInterLJ)=v(ivInterLJ)+U2(rijsq,i,imolty,ii,ntii,j,jmolty,jj,ntjj,ntij)
                                !write(io_output,*) 'CK here checking boltz LJ part',v(ivInterLJ),'rijsq',rijsq
                                !write(io_output,*) 'parameter of pair potential i',i,'imolty',imolty,'ii',ii,'ntii',ntii&
                                !        ,'j',j,'jmolty',jmolty,'jj',jj,'ntjj',ntjj,'ntij',ntij
                             !end if ! CK added
                          end if
                          ...
                          if (L_Coul_CBMC.and.lqchg(ntii).and.lqchg(ntjj).and.rijsq.lt.rcinsq) then
                             ...
                             if (L_elect_table) then
                                ...
                                !write(io_output,*) 'CK here in boltz checking v(ivElect)',v(ivElect)
                             else
                                ! compute all electrostatic interactions
                                v(ivElect) = v(ivElect) + qqu(icharge,ii)*qqu(j,jj)/ rij
                             end if
                             else ! CK added
                                !write(io_output,*) 'CK is here catching ovrlap partII icharge',icharge,'ii',ii,'j',j,'jj',jj
                             end if ! CK added
                          end if
                       end do
                    end do
...
19     if ( my_lovr(my_itrial) ) then
          ...
          my_vtrewald(my_itrial) = v(ivEwald)
           !write(io_output,*) 'CK here in boltz... my_itrial' ,my_itrial
           !write(io_output,*)' CK here in boltz...',v(ivTot), v(ivIntraLJ), v(ivExt), &
           !        v(ivInterLJ),v(ivElect), v(ivEwald)

          if ((my_vtry(my_itrial)*beta).gt.(2.3E0_dp*softcut))then
             !write(io_output,*) 'caught by softcut',my_vtry(my_itrial)*beta
             my_lovr(my_itrial) = .true.
             my_bfac(my_itrial) = 0.0E0_dp
          else if((my_vtry(my_itrial)*beta).lt.-2.303E0_dp*308)then
             !write(io_output,*) '### warning: weight too big out of range',my_vtry(my_itrial)
             my_lovr(my_itrial) = .true.
             my_bfac(my_itrial) = 0.0E0_dp
          else
...
!> calculate the JC correction associated with one bead of one molty in one slab
  function jccoru(bindex,imolty,ii,ibox)
  real::jccoru
  integer,intent(in)::ibox,imolty,bindex,ii
  integer::j,jj,jmolty,ntii,ntjj,ntij,nmcount,indexi
  real::factor,volbin,dz,rbcut,width,halflz,sigma,sigma2,epsilon2,rzuk,drzu,abdrzu,rho,wik,tmp,tmp2,tmp4,tmp10

  dz = boxlz(ibox)/jcbin
  rbcut = rcut(ibox)
  width = boxlz(ibox)/jcbin
  volbin = boxlx(ibox)*boxly(ibox)*dz
  jccoru = 0.0_dp
  halflz = 0.5_dp*boxlz(ibox)

   ntii = ntype(imolty,ii)
   factor = rhoz(bindex,imolty,ii,ibox)*volbin
   do j=1,jcbin
      drzu = (bindex-j)*width
      abdrzu = abs(drzu)
      if (abdrzu .gt. halflz) abdrzu = boxlz(ibox)-abdrzu
      if(abdrzu .le. rbcut) abdrzu=rbcut
      do jmolty =1, nmolty
         do jj =1, nunit(jmolty)
           ntjj=ntype(jmolty,jj)
           ntij=type_2body(ntii,ntjj)
           ...
           sigma2=vvdW(3,ntij)
           epsilon2=vvdW(1,ntij)
           rho = rhoz(j,jmolty,jj,ibox)
           tmp = sigma/abdrzu
           tmp2=tmp*tmp
           tmp4=tmp2*tmp2
           tmp10=tmp4*tmp4*tmp2
           wik = onepi*epsilon2*sigma2*(0.2_dp*tmp10-0.5_dp*tmp4)
           jccoru = jccoru + rho*wik*dz
        end do
      end do
   end do
   jccoru = factor * jccoru
...
subroutine calrhoz(ibox,type)
 integer,intent(in)::ibox,type
 integer::i,ii,j,k,bindex,imolty
 integer::ncount(jcbin,nmolty,40)
 real::rzui,binvol,width,lz,urho
 !initialize
 lz = boxlz(ibox)
 width = lz/jcbin
 binvol = boxlx(ibox)*boxly(ibox)*width
 urho=1.0/binvol
 do bindex = 1,jcbin
   do imolty = 1, nmolty
      do ii = 1,nunit(imolty)
         ncount(bindex,imolty,ii) = 0
      end do
   end do
 end do

 !calculate number of beads in each bin
 do i = 1,nchain
      if(nboxi(i) .ne. ibox) cycle
      imolty = moltyp(i)
      do ii = 1,nunit(imolty)
         rzui = rzu(i,ii)
         if(rzui .lt. 0.0) rzui = rzui + lz
         if(rzui .ge. lz) rzui = rzui - lz
         bindex = int(rzui/width)+1
         ncount(bindex,imolty,ii) = ncount(bindex,imolty,ii) + 1
       end do
 end do

!calculate number density of each bin

  do bindex =1,jcbin
     do imolty=1,nmolty
        do ii=1,nunit(imolty)
           if (type .eq.1) then
               rhoz(bindex,imolty,ii,ibox) = dble(ncount(bindex,imolty,ii))/binvol
           else if (type .eq.2) then  ! calculate final rhoz using sumup
               rhozfi(bindex,imolty,ii,ibox) = dble(ncount(bindex,imolty,ii))/binvol
           end if
        end do
     end do
   end do
end subroutine calrhoz


subroutine calrhoz2(imolty,ibox,type)
integer,intent(in)::ibox,imolty,type
real::volbin,width,rzuio,rzuin,lz
integer::ii,bindexo,bindexn,bindex
lz = boxlz(ibox)
width =  lz/jcbin
volbin = boxlx(ibox)*boxly(ibox)*width
   do ii=1,nunit(imolty)
      rzuio = rzuion(ii,1)
      rzuin = rzuion(ii,2)
      if(rzuio .lt. 0.0) rzuio = rzuio + lz
      if(rzuio .ge. lz) rzuio = rzuio - lz
      if(rzuin .lt. 0.0) rzuin = rzuin + lz
      if(rzuin .ge. lz ) rzuin = rzuin - lz
      bindexo = int(rzuio/width)+1
      bindexn = int(rzuin/width)+1
      if(bindexn .ne. bindexo) then
          if(type .eq. 1) then ! update rhoz
              rhoz(bindexo,imolty,ii,ibox) = rhoz(bindexo,imolty,ii,ibox) - 1.0_dp/volbin
              rhoz(bindexn,imolty,ii,ibox) = rhoz(bindexn,imolty,ii,ibox) + 1.0_dp/volbin
          else if (type .eq. 2) then ! store rhoz
              rhozo(bindexo,imolty,ii,ibox) = rhoz(bindexo,imolty,ii,ibox)
              rhozo(bindexn,imolty,ii,ibox) = rhoz(bindexn,imolty,ii,ibox)
           else if (type .eq. 3) then !restore rhoz
              rhoz(bindexo,imolty,ii,ibox) = rhozo(bindexo,imolty,ii,ibox)
              rhoz(bindexn,imolty,ii,ibox) = rhozo(bindexn,imolty,ii,ibox)
           end if
     end if
   end do

end subroutine calrhoz2

subroutine calrhoz3(ibox,type) !volume move
integer,intent(in)::ibox,type
integer::bindex,imolty,ii

do bindex =1,jcbin
   do imolty=1,nmolty
      do ii=1,nunit(imolty)
         if (type .eq. 2) then!store
             rhozo(bindex,imolty,ii,ibox) = rhoz(bindex,imolty,ii,ibox)
         else if (type .eq. 3) then!restore
             rhoz(bindex,imolty,ii,ibox) = rhozo(bindex,imolty,ii,ibox)
         end if
      end do
   end do
end do

end subroutine calrhoz3

subroutine calrhoz4(lnew,imolty,ibox,type) ! swap
integer,intent(in)::imolty,ibox,type
logical,intent(in)::lnew
integer::ii,bindexn,bindexo
real::width,volbin,rzuin,rzuio,lz
 lz = boxlz(ibox)
 width = lz/jcbin
 volbin = boxlx(ibox) * boxly(ibox) * width
...
if(lnew) then
   do ii=1,nunit(imolty)
      ...
      if(rzuin .ge. lz ) rzuin = rzuin - lz
      bindexn = int(rzuin/width) + 1
      if(type .eq. 1) then !store
        rhozo(bindexn,imolty,ii,ibox) = rhoz(bindexn,imolty,ii,ibox)
      else if (type .eq. 2) then !update
        rhoz(bindexn,imolty,ii,ibox) = rhoz(bindexn,imolty,ii,ibox) + 1.0_dp/volbin
      else if (type .eq. 3) then !restore rhoz
        rhoz(bindexn,imolty,ii,ibox) = rhozo(bindexn,imolty,ii,ibox)
      end if
...
end subroutine calrhoz4

subroutine jcsum(vtail,ibox)
real::vtail,volbin,width,rbcut,tmp,tmp2,tmp4,tmp10,wik,sigma,sigma2
integer::ibox,imolty,bindex,ii,ntii
width = boxlz(ibox)/jcbin
volbin = boxlx(ibox)*boxly(ibox)*width
rbcut = rcut(ibox)
vtail = 0.0_dp
...
do bindex = 1, jcbin
   do imolty=1,nmolty
      do ii=1,nunit(imolty)
         vtail = vtail + jccoru(bindex,imolty,ii,ibox)
      end do
   end do
end do
...
do bindex = 1,jcbin
   do imolty=1,nmolty
      do ii=1,nunit(imolty)
         ...
         tmp4 = tmp2*tmp2
         tmp10 = tmp4 * tmp4 * tmp2
         wik = onepi * vvdW(1,ntii) * sigma2 * (0.2_dp*tmp10-0.5_dp*tmp4)
         correct = correct + rhoz(bindex,imolty,ii,ibox)**2*volbin*width*wik
      end do
   end do
end do


correct = correct * 0.5_dp
vtail = vtail + correct

end subroutine jcsum

! translation,rotation,swatch(same box),cbmc
subroutine energyjc(istart,iuend,vTail,imolty,ibox)
integer,intent(in)::ibox,imolty,istart,iuend
real::lz,volbin,width,wik,vTail,epsilon2,sigma,sigma2,tmp,tmp2,tmp4,tmp10,rbcut,abdrzu,halflz,correct
real::rzui,rzuj,rzuio,rzuin
integer,allocatable::mark(:),bintmp(:),ntypetmp(:)
real,allocatable::rhotmp(:)
integer::num1,num2,ncount
integer::k,m,i,j,ii,jj,jmolty,bindex,bindexi,bindexj,bindexo,bindexn,ntii,ntjj,ntij,sum2
  ...
  allocate(rhotmp(int(num1*2)))
  allocate(bintmp(int(num1*2)))
  allocate(ntypetmp(int(num1*2)))
    m = 1
    do ii = istart,iuend
      rzuio = rzuion(ii,1)
      rzuin = rzuion(ii,2)
      ...
      bindexo = int(rzuio/width) + 1
      bindexn = int(rzuin/width) + 1
      if(bindexo .ne. bindexn) then
         vTail = vTail + jccoru(bindexn,imolty,ii,ibox) + jccoru(bindexo,imolty,ii,ibox)
         bintmp(m) = bindexo
         bintmp(m+1) = bindexn
         rhotmp(m) = rhoz(bindexo,imolty,ii,ibox)
         rhotmp(m+1) = rhoz(bindexn,imolty,ii,ibox)
         ntypetmp(m) = ntype(imolty,ii)
         ntypetmp(m+1) = ntype(imolty,ii)
         m = m + 2
      end if
   end do
! calculate correction term
   ...
   do i=1,m-2
      ...
      do j=i+1,m-1
         ...
         bindexj = bintmp(j)
         ntjj = ntypetmp(j)
         abdrzu = abs(bindexi-bindexj)*width
         if(abdrzu .gt. halflz) abdrzu = lz - abdrzu
         if(abdrzu .le. rbcut)  abdrzu = rbcut
         ntij = type_2body(ntii,ntjj)
         sigma = vvdW(2,ntij)
         sigma2 = vvdW(3,ntij)
         ...
         tmp4=tmp2*tmp2
         tmp10=tmp4*tmp4*tmp2
         wik = onepi*vvdW(1,ntij)*sigma2*(0.2_dp*tmp10-0.5_dp*tmp4)
         correct = correct + rhotmp(i)*rhotmp(j)*volbin*width*wik
     end do
 end do
    vTail = vTail - correct
end subroutine energyjc

! swap move,interbox swatch
subroutine energyjc2(lnew,istart,iuend,vTail,imolty,ibox)
        integer::ibox,imolty,istart,iuend
        logical::lnew
        real::volbin,width,wik,vTail,epsilon2,sigma,sigma2,tmp,tmp2,tmp4,tmp10,rbcut,abdrzu,halflz,correct
        real::rzuio,rzuin,lz,factor
        integer::i,j,ii,jj,jmolty,bindexi,bindexj,bindexn,bindexo,ntii,ntjj,ntij
        integer::bintmp(numax)
        ...
        if(lnew) then
                do ii = istart,iuend
                        ...
                        bindexn = int(rzuin/width) + 1
                        bintmp(ii) = bindexn
                        vTail = vTail + jccoru(bindexn,imolty,ii,ibox)
                end do
        else
                do ii = istart,iuend
                        rzuio = rzuion(ii,1)
                        ...
                        bindexo = int(rzuio/width) + 1
                        bintmp(ii) = bindexo
                        vTail = vTail + jccoru(bindexo,imolty,ii,ibox)
                end do
        end if

        do ii = istart,iuend-1
                ntii = ntype(imolty,ii)
                do jj = ii+1,iuend
                        ntjj = ntype(imolty,jj)
                        ntij = type_2body(ntii,ntjj)
                        sigma = vvdW(2,ntij)
                        sigma2 = vvdW(3,ntij)
                        abdrzu = abs((bintmp(ii)-bintmp(jj))*width)
                        if(abdrzu .ge. halflz) abdrzu = lz - abdrzu
                        if(abdrzu .le. rbcut) abdrzu = rbcut
                        tmp = sigma/abdrzu
                        tmp2=tmp*tmp
                        tmp4=tmp2*tmp2
                        tmp10=tmp4*tmp4*tmp2
                        wik = onepi*vvdW(1,ntij)*sigma2*(0.2_dp*tmp10-0.5_dp*tmp4)
                        correct = correct +  rhoz(bintmp(ii),imolty,ii,ibox)*rhoz(bintmp(jj),imolty,jj,ibox)*wik
                end do
        end do
        correct = factor * correct
        vTail = vTail - correct
end subroutine energyjc2
