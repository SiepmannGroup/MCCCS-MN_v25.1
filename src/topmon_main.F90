...
!> starts and controls the simulation
  subroutine monola(file_in)
    ...
    use sim_particle,only:ctrmas,neighbor,neigh_cnt,neigh_o,save_neighbor_list,neighboro,sort_neighbor_list
    use energy_pairwise,only:sumup,calrhoz
    ...
    integer::io_flt,io_hist,io_cnt,io_ndis,io_config,i,jerr,ibox,itype,itype2,Temp_nmol,nentry,j,nummol,imolty,ii,ntii&
     ,igrow,steps,itemp,jbox,itel,ig,il,nbl,n,zzz,ichkpt,nnn_1st,nstep_per_cycle,bindex
...
! ----------------------------------------------------------------
    ...
    ichkpt = 0
    do ibox=1,nbox
        if(ljc(ibox))  call calrhoz(ibox,1)
    end do
! -----------------------------------------------------------------
...
! save neighbor list before sumup
...
! -------------------------------------------------------------------
    ...
    do ibox=1,nbox
           ...
           do bindex=1,jcbin
               do imolty=1,nmolty
                   do ii=1,nunit(imolty)
                      if( abs ((rhoz(bindex,imolty,ii,ibox) - rhozfi(bindex,imolty,ii,ibox))).gt. 1e-12) then
                          write(io_output,*) '### problem with rhoz update  ###  box ',ibox,bindex,imolty,ii
                          write(io_output,*) 'summup, Markov chain, abs_err:'
                          write(io_output,*) rhoz(bindex,imolty,ii,ibox), rhozfi(bindex,imolty,ii,ibox),rhoz(bindex,imolty,ii,ibox)-rhozfi(bindex,imolty,ii,ibox)
                      end if
                    end do
               end do
            end do
       ...
       ! need to check
       if (myid.eq.rootid) then
          if (( abs((v(ivTot) - vbox(ivTot,ibox))/vbox(ivTot,ibox)) .gt. 1e-11 ).and.( abs((v(ivTot) - vbox(ivTot,ibox))) .gt. 1e-5 )) then      
                write(io_output,*) '### problem with total energy ###  box ',ibox
                write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
                write(io_output,*) v(ivTot),vbox(ivTot,ibox),(v(ivTot)-vbox(ivTot,ibox)),(v(ivTot)-vbox(ivTot,ibox))/abs(vbox(ivTot,ibox))
          end if
           if (( abs((v(ivInterLJ) - vbox(ivInterLJ,ibox))/vbox(ivInterLJ,ibox)).gt. 1e-12).and.( abs((v(ivInterLJ) - vbox(ivInterLJ,ibox))).gt. 1e-6)) then  
             write(io_output,*) '### problem with inter mol.en.  ###'
             write(io_output,*) 'summup, Markov chain, abs_err, rel_err:'
             write(io_output,*) v(ivInterLJ),vbox(ivInterLJ,ibox),(v(ivInterLJ)-vbox(ivInterLJ,ibox)),(v(ivInterLJ)-vbox(ivInterLJ,ibox))/abs(vbox(ivInterLJ,ibox))
             if (lsolid(ibox) .and. .not. lrect(ibox)) then
                write(io_output,*)'You might check the cutoff wrt box widths'
                write(io_output,*) 'Normal PBC might be failing'
             end if
          end if
          if (( abs((v(ivTail) - vbox(ivTail,ibox))/vbox(ivTail,ibox)) .gt. 1e-12).and.( abs((v(ivTail) - vbox(ivTail,ibox))) .gt. 1e-6)) then
          ...
          if (( abs((v(ivIntraLJ) - vbox(ivIntraLJ,ibox))/vbox(ivIntraLJ,ibox)) .gt. 1e-12).and.( abs((v(ivIntraLJ) - vbox(ivIntraLJ,ibox))) .gt. 1e-6)) then
          ...
          if (( abs((v(ivStretching) - vbox(ivStretching,ibox))/vbox(ivStretching,ibox)) .gt. 1e-12).and.( abs((v(ivStretching) - vbox(ivStretching,ibox))) .gt. 1e-6)) then
          ...
          if (( abs((v(ivBending) - vbox(ivBending,ibox))/vbox(ivBending,ibox)) .gt. 1e-9).and.( abs((v(ivBending) - vbox(ivBending,ibox))) .gt. 1e-4)) then
          ...
          if (( abs((v(ivTorsion) - vbox(ivTorsion,ibox))/vbox(ivTorsion,ibox)) .gt. 1e-9).and.( abs((v(ivTorsion) - vbox(ivTorsion,ibox))) .gt. 1e-4)) then
          ...
          if (( abs((v(ivExt) - vbox(ivExt,ibox))/vbox(ivExt,ibox)) .gt. 1e-12).and.( abs((v(ivExt) - vbox(ivExt,ibox))) .gt. 1e-6))then
          ...
          if (( abs((v(ivElect) - vbox(ivElect,ibox))/vbox(ivElect,ibox)) .gt. 1e-12).and.( abs((v(ivElect) - vbox(ivElect,ibox))) .gt. 1e-6)) then
          ...
          if (( abs((v(ivFlucq) - vbox(ivFlucq,ibox))/vbox(ivFlucq,ibox)) .gt. 1e-12).and.( abs((v(ivFlucq) - vbox(ivFlucq,ibox))) .gt. 1e-6)) then
          ...
          if (( abs((v(iv3body) - vbox(iv3body,ibox))/ vbox(iv3body,ibox) ) .gt. 1e-12).and.( abs((v(iv3body) - vbox(iv3body,ibox))) .gt. 1e-6)) then
          ...
          if ( ldielect ) then
             if (( abs((dipolexo - dipolex(ibox))/ dipolex(ibox)) .gt. 1e-12).and.( abs((dipolexo - dipolex(ibox))) .gt. 1e-6))  then
          ...
          if (lmipsw) then
                if((abs((vwellipsw-vbox(ivWellIpswb,ibox))/vbox(ivWellIpswb,ibox)).gt. 1e-12).and.(abs((vwellipsw-vbox(ivWellIpswb,ibox))).gt. 1e-6)) then
...
! check neighbor list update
    if (lneigh.or.lneighbor.or.lgaro) then
        do i=1,nchain
            if (neigh_cnt(i).ne.neigh_o(i)) then !original test only checks the number of neighbors
                write (io_output,*) '### problem with neighbor list ###'
                write (io_output,*) 'molecule: ',i,' molbox: ',nboxi(i),' moltype: ',moltyp(i)
                write (io_output,*) 'num. of nbeighbor from Marcov chain and sumup: ', neigh_o(i), neigh_cnt(i)
                write (io_output,*) ' neighbor list from Marcov chain', neighboro(1:neigh_o(i),i)
                write (io_output,*) ' neighbor list from summup', neighbor(1:neigh_cnt(i),i)
                cycle
            end if
            !HF: makes this sanity check more thorough by checking every element in the neighbor lists
            !We need to first sort the neighbor lists in ascending order for the first neigh_cnt(i) elements
            call sort_neighbor_list(i)
            !now compare the two lists
            do j=1,neigh_cnt(i)
                if (neighbor(j,i).ne.neighboro(j,i)) then
                    write (io_output,*) '### problem with neighbor list ###'
                    write (io_output,*) 'molecule: ',i,' molbox: ',nboxi(i),' moltype: ',moltyp(i)
                    write (io_output,*) 'num. of nbeighbor from Marcov chain and sumup: ', neigh_o(i), neigh_cnt(i)
                    write (io_output,*) ' neighbor list from Marcov chain', neighboro(1:neigh_o(i),i)
          ...
          do i = 1,nchain
             if(nboxi(i).eq.ibox) then
                ...
                do ii = 1,nunit(imolty)
                   ...
                   write(io_config,'(a4,5x,3f15.8)')  chemid(ntii),rxu(i,ii),ryu(i,ii),rzu(i,ii)
                end do
             end if
          end do
          close(io_config)
...
!> \remarks It's usually best to print out information right after it is read in and before doing any other operations (which may fail), so that in the case of program error users will know the progress of input processing
  subroutine readdat(file_in)
    ...
    namelist /analysis/ iprint,imv,iblock,iratp,idiele,iheatcapacity,ianalyze&
     ,nbin,jcbin,lrdf,lintra,lstretch,lgvst,lbend,lete,lrhoz,bin_width&
...
! -------------------------------------------------------------------
    ...
    call mp_bcast(jcbin,1,rootid,groupid)
...
! -------------------------------------------------------------------
    ...
    do i=1,nbox+1
       ljc(i)=.false. !initiallize ljc in everybox to be false 
       if (myid.eq.rootid) then
          call readLine(io_input,line_in,skipComment=.true.,iostat=jerr)
          ...
          read(line_in,*) boxlx(i),boxly(i),boxlz(i),rcut(i),kalp(i),rcutnn(i),numberDimensionIsIsotropic(i),lsolid(i)&
           !,lrect(i),lideal(i),ltwice(i),ltailc(i),ljc(i),rtmp,express(i)
           ,lrect(i),lideal(i),ltwice(i),ljc(i),rtmp,express(i)
          else
          read(line_in,*) boxlx(i),boxly(i),boxlz(i),rcut(i),kalp(i),rcutnn(i),numberDimensionIsIsotropic(i),lsolid(i)&
          ...
          if (.not.ltailc.and.ljc(i)) call err_exit(__FILE__,__LINE__&
           ,'Can not have ljc=T while ltailc=F',myid+1)
          if ((i.ne.1).and.ljc(i)) call err_exit(__FILE__,__LINE__&
           ,'In this version, JC tail corrections is only implement for the first box properly',myid+1)
       ...
       call mp_bcast(ljc(i),1,rootid,groupid)
       ...
       if (lprint) then
          ...
          write(io_output,'(6(A,L2))') '   lsolid: ',lsolid(i),', lrect: ',lrect(i),', lideal: ',lideal(i),', ltwice: '&
           !,ltwice(i),', ltailc: ',ltailc(i),', ljc: ',ljc(i)
           ,ltwice(i),', ljc: ',ljc(i)
...
! -------------------------------------------------------------------
               ...
               ,lelect(imol),lring(imol),lrigid(imol),lbranch(imol),lsetup,lq14scale(imol),qscale(imol),iurot(imol),isolute(imol)
                ...
                if (nunit(imol).gt.numax) then
                   ...
                   if (numax.gt.ubound(ntype,2)) then
                      ...
                      call reallocate(itvib,1,ntmax,1,2*numax,1,nvib_max)
                      call reallocate(ijvib,1,ntmax,1,2*numax,1,nvib_max)
                      call reallocate(inben,1,ntmax,1,2*numax)
                      call reallocate(itben,1,ntmax,1,2*numax,1,nben_max)
                      call reallocate(ijben2,1,ntmax,1,2*numax,1,nben_max)
                      call reallocate(ijben3,1,ntmax,1,2*numax,1,nben_max)
                      call reallocate(intor,1,ntmax,1,2*numax)
                      call reallocate(ittor,1,ntmax,1,2*numax,1,ntor_max)
                      call reallocate(ijtor2,1,ntmax,1,2*numax,1,ntor_max)
                      call reallocate(ijtor3,1,ntmax,1,2*numax,1,ntor_max)
                      call reallocate(ijtor4,1,ntmax,1,2*numax,1,ntor_max)
                ...
                do i = 1, nunit(imol)
                   ...
                   do j = 1, invib(imol,i)
                      ...
                      !else if (brvib(itvib(imol,i,j)).lt.1E-06_dp) then !HF Hsiao-Feng
                      else if (brvib(itvib(imol,i,j)).lt.0.0_dp) then !HF Hsiao-Feng
                         call err_exit(__FILE__,__LINE__,'ERROR: stretching parameters undefined!',myid+1)
                      end if
   ...
   112          continue
    ...
    if (numax.gt.0) then
       ...
       call reallocate(itvib,1,ntmax,1,numax,1,nvib_max)
       call reallocate(ijvib,1,ntmax,1,numax,1,nvib_max)
       call reallocate(inben,1,ntmax,1,numax)
       call reallocate(itben,1,ntmax,1,numax,1,nben_max)
       call reallocate(ijben2,1,ntmax,1,numax,1,nben_max)
       call reallocate(ijben3,1,ntmax,1,numax,1,nben_max)
       call reallocate(intor,1,ntmax,1,numax)
       call reallocate(ittor,1,ntmax,1,numax,1,ntor_max)
       call reallocate(ijtor2,1,ntmax,1,numax,1,ntor_max)
       call reallocate(ijtor3,1,ntmax,1,numax,1,ntor_max)
       call reallocate(ijtor4,1,ntmax,1,numax,1,ntor_max)
...
!> \brief Perform periodic updates and block averages
  subroutine monper()
    ...
    use energy_pairwise,only:energy,coru,calrhoz,jccoru
          ...
          do ibox = 1, nbox
             do k = 1, ncmt(ibox,imolty)
                ...
                do j = 1, nunit(imolty)
                   ...
                   ryuion(j,1) = ryu(i,j)
                   rzuion(j,1) = rzu(i,j)
                   if (myid.eq.rootid) write(io_solute,*) ntype(imolty,j),rxuion(j,1),ryuion(j,1),rzuion(j,1),qquion(j,1)
                end do
...
! -------------------------------------------------------------------
    ...
    call check_rigid_structures()
    !write(io_output,*) "CK bypassed rigid check"
...
! -------------------------------------------------------------------
    ...
    do i=1,nbox
       if (jcbin .gt. 1) then
       write(io_unit,'("! boxlx boxly boxlz rcut kalp rcutnn numDimensionIsIstropic lsolid lrect lideal ltwice ljc temperature &
        &pressure",/,3(F8.3,1X),3(F6.3,1X),I0,1X,5(L1,1X),F7.2,1X,F7.1)') boxlx(i),boxly(i),boxlz(i),rcut(i),kalp(i),rcutnn(i)&
        ,numberDimensionIsIsotropic(i),lsolid(i),lrect(i),lideal(i),ltwice(i),ljc(i),temp,express(i)/MPa2SimUnits
       else
